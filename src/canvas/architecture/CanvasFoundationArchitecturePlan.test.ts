...
describe('Canvas foundation architecture plan', () => {
  ...
  it('keeps the inventory explicit about what can and cannot move', () => {
    ...
    expect(foundationInventory).toContain('compileCanvasAppFoundationExtensions')
    expect(foundationInventory).toContain('executeCanvasAppFoundationExtensionEffects')
    ...
    expect(foundationAdr).toContain('compileCanvasAppFoundationExtensions')
    expect(foundationAdr).toContain('executeCanvasAppFoundationExtensionEffects')
  })

  it('keeps app pressure outside the public foundation contract', () => {
    expect(foundationInventory).toContain(
      'Product pressure stays app-owned until repeated consumer demand appears',
    )
    expect(foundationInventory).toContain('`src/canvas/app`')
  })
})
