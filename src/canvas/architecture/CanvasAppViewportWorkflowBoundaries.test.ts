...
describe('Canvas App viewport workflow boundaries', () => {
  it('keeps app viewport handler wiring behind a named workflow module', () => {
    ...
    const viewportControlsHookFile = getSourceFile(
      'src/canvas/app/affordances/interaction/viewport/useCanvasViewportControls.ts',
    )
    const viewportControlExecutionFile = getSourceFile(
      'src/canvas/app/affordances/interaction/viewport/CanvasViewportControlExecution.ts',
    )
    const wheelViewportHookFile = getSourceFile(
      'src/canvas/app/affordances/interaction/viewport/useCanvasWheelViewport.ts',
    )
    const wheelViewportExecutionFile = getSourceFile(
      'src/canvas/app/affordances/interaction/viewport/CanvasWheelViewportExecution.ts',
    )
    ...
    expect(appModelFile.source).not.toContain(
      "from '../affordances/interaction/viewport/useCanvasWheelViewport'",
    )
    expect(appModelFile.source).not.toContain(
      "from '../affordances/interaction/viewport/useCanvasViewportControls'",
    )
