...
8. `CanvasAffordanceCatalog` owns the built-in Affordance group list and default feature toggle matrix. `CanvasAffordanceConfig` owns catalog-driven config merge, validation, and snapshot behavior, with public affordance id types derived from the catalog. `CanvasAffordanceMetadata` owns immutable built-in tool/command labels, status labels, and toolbar tool order so UI modules consume the Engine affordance metadata instead of repeating built-in tool lists.
