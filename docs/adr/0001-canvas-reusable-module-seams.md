...
8. `CanvasAffordanceCatalog` owns the built-in Affordance group list and default feature toggle matrix. `CanvasAffordanceConfig` owns config merge, validation, and snapshot behavior, with public affordance id types derived from the catalog.
