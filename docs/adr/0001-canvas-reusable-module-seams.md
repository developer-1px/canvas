...
3. `src/canvas/index.ts` is the package public entry for external assembly and Demo code. `src/canvas/app/index.ts` is the App public facade. Demo modules import `src/canvas`, not canvas implementation subpaths. The package manifest opens only the package entry and layer facade subpaths, each pointing at a public `index.ts`, preserves CSS imports through `sideEffects`, and package self-import smoke tests verify those exports from a consumer perspective.
4. `src/canvas/app/workflow` owns App Model modules. Workspace, interaction, text editor, and find/replace state are hidden behind workflow hooks before props reach the App Shell.
