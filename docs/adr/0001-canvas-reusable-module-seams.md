...
7. `CanvasAppCustomItemModule` bundles the creation tool, renderer, validator, inspector panel, and commands for one product-specific item kind so adding a new kind has one definition unit. Each module has a stable `id`, can be disabled by Host App assembly, and duplicate or unknown extension keys fail at assembly time instead of being overwritten or ignored.
