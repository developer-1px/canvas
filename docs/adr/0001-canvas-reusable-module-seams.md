...
- Renderer Stage knowing Whiteboard `CanvasItem` variants and Host bounds helpers.
...
1. `src/canvas/renderer` owns SVG stage and overlay orchestration only. It accepts an injected item layer and does not import Whiteboard `CanvasItem`, Host read helpers, or the Canvas Component Library.
2. `src/canvas/app/rendering` owns the Whiteboard SVG Item Layer Adapter. It converts Whiteboard `CanvasItem` trees and component presentation keys into SVG children for the Renderer Adapter.
...
6. Host document behavior is exposed through the explicit `CanvasDocumentController` interface. json-document document, JSON Patch, selection snapshot, and clipboard internals stay inside `src/canvas/host/document`.
7. Built-in whiteboard tools such as sticky notes, pen path, marker, highlighter, eraser, arrow, bounded shapes(rectangle/ellipse/diamond), checklist editing, Queue/Kanban editing, table import, link preview import, comments, reaction stamps, and transient emotes stay internal Affordances, not product-specific custom item modules. Sticky note creation reuses the Component Library `sticky` template instead of introducing another stable item variant, and Host sticky/text rules own blank sticky creation, quick-create, and body editing through shared contracts. Checklist editing keeps item check/text/add/remove UI in the App checklist Inspector, while Host checklist component rules own checked index normalization and item text/add/remove/toggle storage changes. Queue/Kanban editing keeps card text/add/remove/reorder UI in the App kanban Inspector, while Host kanban component rules own card normalization, ordering, and height synchronization. Table import keeps CSV/TSV/file/paste/drop browser IO in the App table module, while Host table component rules own rows-to-component normalization and dynamic grid calculation. Link preview import keeps URL/embed paste/drop browser IO and Inspector actions in the App link module, while Host link preview component rules own HTTP/HTTPS URL normalization, preview item creation, source text conversion, and horizontal/vertical orientation storage. Bounded shapes use the shared `create-shape` pointer grammar and existing bounded shape item storage with a `shape` field so older rect documents remain valid. Host shape geometry owns shape-to-SVG primitive mapping so item renderers do not repeat concrete shape kind branches. Host tree/document code owns drawing canonical bounds and derives `x/y/w/h` from stroke `points`, path `segments`, or arrow `start/end` before storage. Host drawing geometry owns arrow connector endpoint attachment movement and elbow/straight routing storage changes, while Host comment/stamp modules own their persisted item validation and Host attachment owns shared whole-item `attachedTo` movement rules.
...
- Adding a whiteboard component kind that reuses an existing presentation remains a Canvas Component Library change.
