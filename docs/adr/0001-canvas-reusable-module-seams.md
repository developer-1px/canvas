...
7. `CanvasAppCustomItemModule` bundles the creation tool, renderer, validator, inspector panel, and commands for one product-specific item kind so adding a new kind has one definition unit. Each module has a stable `id`, and duplicate extension keys fail at assembly time instead of being overwritten.
