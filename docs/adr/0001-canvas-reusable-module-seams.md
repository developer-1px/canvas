...
7. Built-in whiteboard tools such as sticky notes, marker, highlighter, arrow, comments, and reactions stay internal Affordances, not product-specific custom item modules. Sticky note creation reuses the Component Library `sticky` template instead of introducing another stable item variant, and Host text rules own sticky body editing through the shared editable text contract. Host tree/document code owns drawing canonical bounds and derives `x/y/w/h` from `points` or `start/end` before storage. Host comment/stamp modules own their persisted item validation, and Host attachment owns shared `attachedTo` movement rules so collaboration affordances can follow selected objects without type-specific transform branches.
