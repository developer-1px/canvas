...
## Text Paste Import Rich Text Contract

| Area | Contract |
| --- | --- |
| Rich text source | HTML or Markdown paste may produce `CanvasRichTextPasteSource` with paragraph/run metadata before host text replacement |
| Rich text formats | Source `format` is `text-html-rich` for HTML and `text-markdown-rich` for Markdown |
| Markdown metadata | Markdown headings use `headingLevel`; inline code uses `run.code`; Markdown links reuse link/color/underline run metadata |
...
| Area | Contract |
| --- | --- |
| HTML source | `text/html` may produce a batch of `CanvasImageImportSource` values from every `<img src="data:image/...">` in document order |
| Source shape | Non-SVG data image URLs keep `format: 'data-url-html-img'`; SVG data image URLs reuse `format: 'svg-html-img'`, sanitization, size extraction, and name rules |
| Name metadata | `alt` wins over `title`; extension normalization follows the existing single image source helper |
| Dedupe | Dedupe uses the normalized `dataUrl` as the key and keeps the first occurrence |
...
| `src/canvas/app/pointer/CanvasPointerCreationPreview.ts` | Create-shape, pen path, marker, highlighter, arrow, custom creation의 draft overlay와 currentWorld/moved preview를 소유한다 |
...
| `src/canvas/host/component/CanvasTableComponent.ts` | Built-in table component kind, CSV rows 정규화, 동적 column/body grid, table item size/default style 계약을 소유한다 |
| `src/canvas/host/component/CanvasLinkPreviewComponent.ts` | Built-in link preview component kind, URL 정규화, domain fallback title, 저장 가능한 preview item 계약을 소유한다 |
...
- Sticky note, pen path, marker, highlighter, eraser, arrow, bounded shapes(rectangle/ellipse/diamond), checklist editing, Queue/Kanban editing, table import, link preview import, comment, reaction stamp, laser pointer, cursor chat, emote burst, session timer, voting session은 제품별 custom item이 아니라 내부 Affordance다. Sticky note는 별도 entity 없이 Component Library의 `sticky` template을 component item으로 생성하고, sticky body 편집, quick-create, 텍스트 기반 세로 auto-grow는 Host sticky/editable text contract를 재사용한다. Checklist editing은 App checklist Inspector가 UI를 숨기고 Host checklist component contract가 checked index 정규화와 item text/add/remove/toggle 저장 규칙을 소유한다. Queue/Kanban editing은 App kanban Inspector가 UI를 숨기고 Host kanban component contract가 card text/add/remove/reorder와 높이 보정 저장 규칙을 소유한다. Table import는 App table module이 CSV/TSV/file/paste/drop IO를 숨기고 Host table component contract가 rows-to-component 정규화와 grid 계산을 소유한다. Link preview import는 App link module이 URL/embed paste/drop IO와 Inspector action을 숨기고 Host link preview component contract가 HTTP/HTTPS URL 정규화, preview item 생성, source text 변환, horizontal/vertical orientation 저장 규칙을 소유한다. Bounded shapes는 `create-shape` pointer grammar와 기존 bounded shape item의 `shape` 계약으로 저장해 rect 문서 호환성을 유지한다. Comment body 표시와 편집도 Host comment/editable text contract가 소유한다. Reaction stamp는 선택 객체에 붙어도 선택을 훔치지 않고, 같은 target의 stamp끼리 겹치지 않게 App stamp insertion이 배치한다. Active voting session은 stamp insertion에 quota만 주입하고 vote session 자체는 document item으로 저장하지 않는다. Laser pointer는 document item을 만들지 않고 App pointer interaction과 Engine overlay state로만 유지된다. Cursor chat, emote burst, session timer, voting session은 문서 item을 만들지 않고 App UI transient state로만 유지된다. Drawing Item의 `x/y/w/h`는 외부 입력이 아니라 marker/highlighter `points`, path `segments`, arrow `start/end`에서 Host tree/document가 동기화하는 canonical bounds다. Path item은 Core `Point`만 참조하는 typed move/line/cubic segment storage를 Host item contract로 저장하고, SVG `d` 문자열은 Renderer primitive가 파생한다. App pointer lifecycle은 `pen` tool을 `draw-path` gesture로 라우팅하고 draft stroke preview를 재사용한 뒤 commit에서 typed path item을 만든다. App rendering은 path 선택 시 anchor handle affordance만 표시하고 point-level path editing은 별도 App interaction으로 확장한다. Arrow connector endpoint attachment와 elbow/straight routing 저장 변경은 Host Drawing Item Geometry가 소유한다. Drawing Item style 기본값은 Host Drawing Item Style Module이 소유하고 draft overlay와 item creation이 같은 값을 쓴다. Marker/highlighter 렌더링은 connector path와 분리된 freehand smoothing primitive를 쓰고, eraser는 freehand stroke를 whole-stroke document removal로 처리한다.
