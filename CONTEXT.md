...
- Canvas Stamp Item: 공통 화이트보드 반응/스탬프 persisted item type은 Entities Contract가 소유하고, stable stamp id, visible label, positive bounds 저장 shape 검증은 Host-owned stamp Module이 소유한다.
...
- Canvas Stamp Insertion: stamp definition, selection-adjacent/viewport-center placement, id generation, document add commit, post-insert selection을 Canvas Stamp Item 삽입 결과로 바꾸는 App-owned stamp Module.
...
- Stamp/reaction의 stable item type은 Entities Contract가 소유하고, stamp id/label/bounds 저장 shape 검증은 Host Canvas Stamp Item Module이 소유한다.
