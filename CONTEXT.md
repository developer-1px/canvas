...
- Canvas Keyboard Command Dispatch: Keyboard document command intent runner table을 소유하고, 지원 intent 판정과 command handler bundle 호출을 같은 table에서 파생하는 App-owned Module.
- Canvas Keyboard System Dispatch: Keyboard system intent runner table을 소유하고, 지원 system intent 판정과 system handler bundle 호출을 같은 table에서 파생하며 temporary pan release를 처리하는 App-owned Module.
- Canvas Keyboard Viewport Dispatch: Keyboard viewport intent runner table을 소유하고, 지원 viewport intent 판정과 viewport handler bundle 호출을 같은 table에서 파생하는 App-owned Module.
