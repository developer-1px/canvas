...
- Canvas Clipboard Command Handlers: clone, duplicate, copy, paste, cut callback을 clipboard command descriptor로 변환하고 current paste index injection과 duplicate source selection default를 소유하는 App-owned handler grammar Module.
...
- Clipboard command hook은 paste index ref, execution context, runner memoization을 맡고, clone/duplicate/copy/paste/cut callback grammar와 paste index injection은 Canvas Clipboard Command Handlers가 소유한다. Canvas Clipboard Command Execution은 plan 생성과 effect 적용만 조립한다. Clone/duplicate/paste/cut plan과 paste offset 계산은 Canvas Clipboard Command Effect Plan이, Host clipboard/document/editing effect routing은 Canvas Clipboard Command Effects가 소유한다.
