...
- React Component Slot: component root의 직계 DesignDocument child가 같은 definition/instance와 고유 slot id로 연결될 때 React renderer에 전달되는 명명된 ReactNode. renderer가 배치를 소유해도 slot node의 stable id, DOM projection, 선택, 개별 편집은 유지되며 binding이 없으면 authored children이 fallback이다.
- React Component Instance Factory: component slot tree를 root부터 순서대로 추가하는 하나의 undoable DesignDocument command로 만드는 ReactDesign public factory. definition/instance/slot binding을 주입하고 node id와 slot id 중복을 command 생성 전에 거부한다.
