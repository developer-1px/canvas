import { expect, test, type Page } from '@playwright/test'

const DESIGN_SYSTEM_QA_VIEWPORTS = [
  { height: 1152, minSpecimenWidth: 740, width: 2048 },
  { height: 900, minSpecimenWidth: 640, width: 1440 },
  { height: 800, minSpecimenWidth: 500, width: 1280 },
  { height: 768, minSpecimenWidth: 420, width: 1024 },
  { height: 844, minSpecimenWidth: 318, width: 390 },
]

test('opens as a source-linked design system specimen demo', async ({
  page,
}) => {
  await page.goto('/')
  await page.evaluate(() => {
    window.addEventListener('html-specimen-html:export', (event) => {
      const targetWindow = window as Window & {
        __htmlSpecimenExportedHtml?: string
      }
      const detail = (event as CustomEvent<{ html?: unknown }>).detail

      targetWindow.__htmlSpecimenExportedHtml =
        typeof detail.html === 'string' ? detail.html : ''
    })
    window.addEventListener('html-specimen-css:export', (event) => {
      const targetWindow = window as Window & {
        __htmlSpecimenExportedCss?: string
      }
      const detail = (event as CustomEvent<{ css?: unknown }>).detail

      targetWindow.__htmlSpecimenExportedCss =
        typeof detail.css === 'string' ? detail.css : ''
    })
  })

  await expect(page.getByText('Evidence Map')).toHaveCount(0)
  await expect(page.getByText('Target outcomes')).toHaveCount(0)
  await expect(page.getByText('AI change review')).toHaveCount(0)
  await expect(page.getByRole('toolbar', { name: 'Tools' })).toHaveCount(0)
  const viewportControls = page.getByRole('toolbar', {
    name: 'Canvas viewport',
  })

  await expect(viewportControls).toBeVisible()
  const initialScale = await readCanvasScalePercent(page)

  await viewportControls.getByRole('button', { name: 'Zoom in' }).click()
  await expect.poll(() => readCanvasScalePercent(page))
    .toBeGreaterThan(initialScale)
  await viewportControls.getByRole('button', { name: 'Pan canvas' }).click()
  await expect(viewportControls.getByRole('button', { name: 'Pan canvas' }))
    .toHaveAttribute('aria-pressed', 'true')
  await expect(page.locator('.selection-floating-bar')).toHaveCount(0)

  const preview = page.locator('.demo-html-specimen-preview').first()

  await expect(preview).toBeVisible()
  await expect(preview.getByText('Release queue')).toBeVisible()
  await expect(preview).toHaveAttribute('data-preview-target-node-id', /dom:/)
  await expect(page.locator('.devtools-selection-dock')).toBeVisible()
  await expect(page.locator('.object-inspector-devtools')).toHaveCount(0)
  await expect(page.locator('.html-specimen-target-panel summary'))
    .toContainText('div.table-row.release-row')
  await page.locator('.html-specimen-target-panel summary').click()
  await expect(page.locator('.html-specimen-css-target')).toContainText(
    'div.table-row.release-row',
  )
  await expect(page.locator('.html-specimen-node-facts')).toContainText(
    'ReleaseRow',
  )
  await expect(page.locator('.html-specimen-node-facts')).toContainText(
    'src/app/design-system/Button.tsx',
  )
  await expect(page.locator('.html-specimen-outline-tree')).toContainText(
    'DesignSystemReleaseQueue',
  )
  await expect(page.locator('.html-specimen-outline-tree')).toContainText(
    'section.queue-toolbar',
  )
  await expect(page.locator('.html-specimen-target-panel summary'))
    .toContainText('Target')
  await expect(page
    .locator('.html-specimen-outline-row[aria-expanded="true"]')
    .first()).toBeVisible()
  await expect(page.locator('.html-specimen-target-panel')).toBeVisible()
  await expect(page.locator('.html-specimen-style-panel')).toBeVisible()
  await expect.poll(async () =>
    page.locator('.html-specimen-outline-tree').evaluate((element) =>
      Math.round(element.getBoundingClientRect().height),
    ),
  ).toBeLessThanOrEqual(140)
  await expect(page.locator('.html-specimen-actions-strip')).toContainText(
    'Apply patch',
  )
  await expect(page
    .locator('.html-specimen-actions-strip')
    .getByRole('button', { name: 'Copy patch' })).toBeVisible()
  await expect(page
    .locator('.html-specimen-actions-strip')
    .getByRole('button', { name: 'Export HTML' })).toBeVisible()
  await expect(page
    .locator('.html-specimen-actions-strip')
    .getByRole('button', { name: 'Export CSS' })).toBeVisible()
  await expect(page
    .locator('.html-specimen-actions-strip')
    .getByRole('button', { name: 'Revert changes' })).toBeVisible()
  await expect(page.locator('.html-specimen-css-patch-preview')).toContainText(
    '.release-row',
  )
  await expect(page.getByRole('button', { name: 'Copy HTML' })).toHaveCount(0)
  await expect(page.getByRole('button', { name: 'Copy CSS' })).toHaveCount(0)
  await expect(cssField(page, 'border-radius')
    .locator('input')).toHaveValue('5px')

  await page
    .locator('.html-specimen-actions-strip')
    .getByRole('button', { name: 'Export HTML' })
    .click()
  await page
    .locator('.html-specimen-actions-strip')
    .getByRole('button', { name: 'Export CSS' })
    .click()

  await expect.poll(async () =>
    page.evaluate(() =>
      (window as Window & {
        __htmlSpecimenExportedHtml?: string
      }).__htmlSpecimenExportedHtml ?? ''),
  ).toContain('data-surface-component="DesignSystemReleaseQueue"')
  await expect.poll(async () =>
    page.evaluate(() =>
      (window as Window & {
        __htmlSpecimenExportedHtml?: string
      }).__htmlSpecimenExportedHtml ?? ''),
  ).not.toContain('data-surface-props')
  await expect.poll(async () =>
    page.evaluate(() =>
      (window as Window & {
        __htmlSpecimenExportedCss?: string
      }).__htmlSpecimenExportedCss ?? ''),
  ).toContain('--ds-primary')
})

test('keeps the design system demo contained across QA viewports', async ({
  page,
}) => {
  for (const viewport of DESIGN_SYSTEM_QA_VIEWPORTS) {
    await page.setViewportSize({
      height: viewport.height,
      width: viewport.width,
    })
    await page.goto('/')
    await expect(page.locator('.html-specimen-target-panel summary'))
      .toContainText('div.table-row.release-row')

    const metrics = await readDesignSystemDemoMetrics(page)

    expect(metrics.target).toBe('div.table-row.release-row')
    expect(metrics.specimen?.width).toBeGreaterThanOrEqual(
      viewport.minSpecimenWidth,
    )
    expect(metrics.dock?.width).toBeGreaterThanOrEqual(300)
    expect(metrics.dock?.height).toBeLessThan(viewport.height)
    expect(metrics.sideInspector).toBe(0)
    expect(metrics.canvasOrigin).toBe(true)
    expect(metrics.canvasScale).toBe(true)
    expect(metrics.overlayAligned).toBe(true)
    expect(metrics.toolbar).toBe(0)
    expect(metrics.floatingBar).toBe(0)
    expect(metrics.itemOutline).toBe(0)
    expect(metrics.resizeHandles).toBe(0)
    expect(metrics.zoomControls).toBe(1)
  }
})

test('selects concrete HTML elements from preview and outliner', async ({
  page,
}) => {
  await page.setViewportSize({ height: 900, width: 1440 })
  await page.goto('/')
  await page.locator('.html-specimen-target-panel summary').click()

  await page
    .locator('.html-specimen-outline-row-button')
    .filter({ hasText: 'div.table-row.release-row' })
    .first()
    .click()
  await expect(page.locator('.html-specimen-css-target')).toContainText(
    'div.table-row.release-row',
  )

  await dispatchPreviewPointerDown(page, '.button.primary')
  await expect(page.locator('.html-specimen-css-target')).toContainText(
    'button.button.primary',
  )

  await clickPreviewElement(page, '.status.blocked')
  await expect(page.locator('.html-specimen-css-target')).toContainText(
    'span.status.blocked',
  )

  await dispatchPreviewPointerDown(page, '.filter-bar')
  await expect(page.locator('.html-specimen-css-target')).toContainText(
    'section.filter-bar',
  )

  await page.setViewportSize({ height: 768, width: 1024 })
  await page.goto('/')
  await dispatchPreviewPointerDown(page, '.search-field')
  await expect(page.locator('.html-specimen-css-target')).toContainText(
    'label.search-field',
  )
})

test('drives the HTML outline with keyboard selection and edit shortcuts', async ({
  page,
}) => {
  await page.setViewportSize({ height: 900, width: 1440 })
  await page.goto('/')

  const preview = page.locator('.demo-html-specimen-preview').first()
  const outline = page.locator('.html-specimen-outline-tree')
  const target = page.locator('.html-specimen-css-target')
  const shortcutMod = process.platform === 'darwin' ? 'Meta' : 'Control'

  await expect(preview).toBeVisible()
  await page.locator('.html-specimen-target-panel summary').click()
  await page
    .locator('.html-specimen-outline-row-button')
    .filter({ hasText: 'button.button.primary' })
    .first()
    .click()
  await outline.focus()
  await expect(outline).toBeFocused()
  await expect(target).toContainText('button.button.primary')

  await page.keyboard.press('ArrowUp')
  await expect(target).toContainText('button.button.secondary')
  await page.keyboard.press('ArrowDown')
  await expect(target).toContainText('button.button.primary')
  await page.keyboard.press('ArrowLeft')
  await expect(target).toContainText('div.toolbar-actions')
  await page.keyboard.press('ArrowRight')
  await expect(target).toContainText('button.button.secondary')

  await page
    .locator('.html-specimen-outline-row-button')
    .filter({ hasText: 'button.button.primary' })
    .first()
    .click()
  await expect(outline).toBeFocused()
  await expect(target).toContainText('button.button.primary')

  await page.keyboard.press('Enter')
  await expect(page.locator('.html-specimen-outline-input'))
    .toHaveValue('Approve selected')
  await page.locator('.html-specimen-outline-input').fill('Ship')
  await page.keyboard.press('Enter')
  await expect(preview.locator('.queue-toolbar .button.primary'))
    .toHaveText('Ship')
  await expect(outline).toBeFocused()

  const primaryButtons = preview.locator('.button.primary')
  await expect(primaryButtons).toHaveCount(2)
  await page.keyboard.press(`${shortcutMod}+C`)
  await page.keyboard.press(`${shortcutMod}+V`)
  await expect(primaryButtons).toHaveCount(3)
  await expect(target).toContainText('button.button.primary')

  await page.keyboard.press(`${shortcutMod}+D`)
  await expect(primaryButtons).toHaveCount(4)
  await expect(target).toContainText('button.button.primary')

  await page.keyboard.press(`${shortcutMod}+X`)
  await expect(primaryButtons).toHaveCount(3)
  await expect(target).toContainText('button.button.primary')

  await page.keyboard.press(`${shortcutMod}+V`)
  await expect(primaryButtons).toHaveCount(4)
  await expect(target).toContainText('button.button.primary')

  await page.keyboard.press('Delete')
  await expect(primaryButtons).toHaveCount(3)
})

test('pastes HTML/CSS and edits preview target CSS through the inspector', async ({
  page,
}) => {
  await page.goto('/')
  await page.evaluate(() => {
    window.addEventListener('html-specimen-html:export', (event) => {
      const targetWindow = window as Window & {
        __htmlSpecimenExportedHtml?: string
      }
      const detail = (event as CustomEvent<{ html?: unknown }>).detail

      targetWindow.__htmlSpecimenExportedHtml =
        typeof detail.html === 'string' ? detail.html : ''
    })
    window.addEventListener('html-specimen-css:export', (event) => {
      const targetWindow = window as Window & {
        __htmlSpecimenExportedCss?: string
      }
      const detail = (event as CustomEvent<{ css?: unknown }>).detail

      targetWindow.__htmlSpecimenExportedCss =
        typeof detail.css === 'string' ? detail.css : ''
    })
  })

  const pasteWasHandled = await page.evaluate((text) => {
    const data = new DataTransfer()

    data.setData('text/plain', text)

    const event = new ClipboardEvent('paste', {
      bubbles: true,
      cancelable: true,
      clipboardData: data,
    })

    window.dispatchEvent(event)

    return event.defaultPrevented
  }, JSON.stringify({
    css: [
      '.button {',
      '  border: 1px solid transparent;',
      '  border-color: transparent;',
      '  border-radius: 6px;',
      '  font-size: 14px;',
      '  margin: 0;',
      '  padding: 12px 18px;',
      '}',
      '.primary {',
      '  color: #ffffff;',
      '  background: #2563eb;',
      '}',
    ].join('\n'),
    html: [
      '<main>',
      '<button id="primary" class="button primary">Save</button>',
      '<button id="secondary" class="button secondary">Cancel</button>',
      '<span id="orphan">Loose</span>',
      '</main>',
    ].join(''),
  }))

  expect(pasteWasHandled).toBe(true)

  const preview = page.locator('.demo-html-specimen-preview').last()

  await expect(preview).toBeVisible()
  await preview.locator('button#primary').hover()

  await expect(preview).toHaveAttribute('data-preview-hover-node-id', /dom:/)
  await expect.poll(async () =>
    preview.evaluate((host) => {
      const hovered = host.shadowRoot
        ?.querySelector('[data-html-specimen-preview-hover]')

      return hovered instanceof HTMLElement ? hovered.id : ''
    }),
  ).toBe('primary')
  await expect.poll(async () =>
    preview.evaluate((host) => {
      const overlay = host.shadowRoot
        ?.querySelector('[data-html-specimen-preview-overlay="hover"]')

      return overlay instanceof HTMLElement
        ? {
            height: Number.parseFloat(overlay.style.height),
            nodeId: overlay.dataset.previewNodeId ?? '',
            width: Number.parseFloat(overlay.style.width),
          }
        : null
    }),
  ).toMatchObject({
    height: expect.any(Number),
    nodeId: expect.stringMatching(/^dom:/),
    width: expect.any(Number),
  })

  await preview.locator('button#primary').click()

  await expect(preview).toHaveAttribute('data-preview-target-node-id', /dom:/)
  await expect.poll(async () =>
    preview.evaluate((host) => {
      const overlay = host.shadowRoot
        ?.querySelector('[data-html-specimen-preview-overlay="target"]')

      return overlay instanceof HTMLElement
        ? overlay.dataset.previewNodeId ?? ''
        : ''
    }),
  ).toMatch(/^dom:/)
  await expect.poll(async () =>
    preview.evaluate((host) => {
      return host.shadowRoot?.querySelector(
        '[data-html-specimen-preview-spacing]',
      ) === null
    }),
  ).toBe(true)
  await expect(cssField(page, 'border-radius')
    .getByText('.button · 2 nodes')).toBeVisible()
  await expect(cssField(page, 'font-size')
    .locator('input')).toHaveValue('14px')

  await page.locator('.html-specimen-target-panel summary').click()
  await page
    .locator('.html-specimen-outline-row-button')
    .filter({ hasText: 'Save' })
    .first()
    .dblclick()
  await expect(page.locator('.html-specimen-outline-input'))
    .toHaveValue('Save')
  await page.locator('.html-specimen-outline-input').fill('Launch')
  await page.locator('.html-specimen-outline-input').press('Enter')

  await expect(preview.locator('button#primary')).toHaveText('Launch')
  await expect(page.locator('.html-specimen-css-changes')).toContainText(
    'Launch',
  )
  await expect(page
    .locator('.html-specimen-content-field')
    .filter({ hasText: 'Text' })
    .locator('input')).toHaveValue('Launch')

  const textInput = cssField(page, 'color')
    .locator('input')

  await expect(textInput).toBeVisible()
  await textInput.fill('#facc15')
  await textInput.blur()

  await expect.poll(async () =>
    preview.evaluate((host) =>
      host.shadowRoot?.querySelector('style')?.textContent ?? ''),
  ).toContain('color: #facc15;')
  await expect(page.locator('.html-specimen-css-patch-preview')).toContainText(
    'color: #facc15;',
  )
  await expect(page.locator('.html-specimen-css-changes')).toContainText(
    '#facc15',
  )
  await expect.poll(async () =>
    preview.locator('button#primary').evaluate((button) =>
      getComputedStyle(button).color),
  ).toBe('rgb(250, 204, 21)')
  await expect(cssField(page, 'color')
    .getByText('.primary · 1 node')).toBeVisible()

  const backgroundInput = cssField(page, 'background-color')
    .locator('input')

  await expect(backgroundInput).toBeVisible()
  await backgroundInput.fill('#111827')
  await backgroundInput.blur()

  await expect.poll(async () =>
    preview.evaluate((host) =>
      host.shadowRoot?.querySelector('style')?.textContent ?? ''),
  ).toContain('background: #111827;')
  await expect.poll(async () =>
    preview.evaluate((host) =>
      host.shadowRoot?.querySelector('style')?.textContent ?? ''),
  ).not.toContain('background-color: #111827;')
  await expect.poll(async () =>
    preview.locator('button#primary').evaluate((button) =>
      getComputedStyle(button).backgroundColor),
  ).toBe('rgb(17, 24, 39)')
  await expect(cssField(page, 'background-color')
    .getByText('.primary · 1 node')).toBeVisible()

  const strokeInput = cssField(page, 'border-color')
    .locator('input')

  await expect(strokeInput).toBeVisible()
  await strokeInput.fill('#f97316')
  await strokeInput.blur()

  await expect.poll(async () =>
    preview.evaluate((host) =>
      host.shadowRoot?.querySelector('style')?.textContent ?? ''),
  ).toContain('border-color: #f97316;')
  await expect.poll(async () =>
    preview.locator('button#primary').evaluate((button) =>
      getComputedStyle(button).borderTopColor),
  ).toBe('rgb(249, 115, 22)')
  await expect(cssField(page, 'border-color')
    .getByText('.button · 2 nodes')).toBeVisible()

  const fontInput = cssField(page, 'font-size')
    .locator('input')

  await expect(fontInput).toBeVisible()
  await fontInput.fill('18px')
  await fontInput.blur()

  await expect.poll(async () =>
    preview.evaluate((host) =>
      host.shadowRoot?.querySelector('style')?.textContent ?? ''),
  ).toContain('font-size: 18px;')
  await expect.poll(async () =>
    preview.locator('button#primary').evaluate((button) =>
      getComputedStyle(button).fontSize),
  ).toBe('18px')
  await expect(cssField(page, 'font-size')
    .getByText('.button · 2 nodes')).toBeVisible()

  const radiusInput = cssField(page, 'border-radius')
    .locator('input')

  await expect(radiusInput).toBeVisible()
  await radiusInput.fill('12px')
  await radiusInput.blur()

  await expect.poll(async () =>
    preview.evaluate((host) =>
      host.shadowRoot?.querySelector('style')?.textContent ?? ''),
  ).toContain('border-radius: 12px;')
  await expect.poll(async () =>
    preview.locator('button#primary').evaluate((button) =>
      getComputedStyle(button).borderTopLeftRadius),
  ).toBe('12px')
  await expect(cssField(page, 'border-radius')
    .getByText('.button · 2 nodes')).toBeVisible()

  const paddingInput = cssField(page, 'padding')
    .locator('input')

  await expect(paddingInput).toBeVisible()
  await paddingInput.fill('8px 16px')
  await paddingInput.blur()

  await expect.poll(async () =>
    preview.evaluate((host) =>
      host.shadowRoot?.querySelector('style')?.textContent ?? ''),
  ).toContain('padding: 8px 16px;')
  await expect.poll(async () =>
    preview.locator('button#primary').evaluate((button) => {
      const style = getComputedStyle(button)

      return `${style.paddingTop} ${style.paddingLeft}`
    }),
  ).toBe('8px 16px')
  await expect(cssField(page, 'padding')
    .getByText('.button · 2 nodes')).toBeVisible()
  await expect.poll(async () =>
    preview.evaluate((host) => {
      return host.shadowRoot?.querySelector(
        '[data-html-specimen-preview-spacing]',
      ) === null
    }),
  ).toBe(true)

  const marginInput = cssField(page, 'margin')
    .locator('input')

  await expect(marginInput).toBeVisible()
  await marginInput.fill('4px')
  await marginInput.blur()

  await expect.poll(async () =>
    preview.evaluate((host) =>
      host.shadowRoot?.querySelector('style')?.textContent ?? ''),
  ).toContain('margin: 4px;')
  await expect.poll(async () =>
    preview.locator('button#primary').evaluate((button) =>
      getComputedStyle(button).marginTop),
  ).toBe('4px')
  await expect.poll(async () =>
    preview.evaluate((host) => {
      return host.shadowRoot?.querySelector(
        '[data-html-specimen-preview-spacing]',
      ) === null
    }),
  ).toBe(true)
  await clickInspectorExport(page, 'CSS')
  await expect.poll(async () =>
    page.evaluate(() =>
      (window as Window & {
        __htmlSpecimenExportedCss?: string
      }).__htmlSpecimenExportedCss ?? ''),
  ).toContain('background: #111827;')
  await expect.poll(async () =>
    page.evaluate(() =>
      (window as Window & {
        __htmlSpecimenExportedCss?: string
      }).__htmlSpecimenExportedCss ?? ''),
  ).not.toContain('background-color: #111827;')
  await expect.poll(async () =>
    page.evaluate(() =>
      (window as Window & {
        __htmlSpecimenExportedCss?: string
      }).__htmlSpecimenExportedCss ?? ''),
  ).toContain('color: #facc15;')
  await expect.poll(async () =>
    page.evaluate(() =>
      (window as Window & {
        __htmlSpecimenExportedCss?: string
      }).__htmlSpecimenExportedCss ?? ''),
  ).toContain('font-size: 18px;')
  await expect.poll(async () =>
    page.evaluate(() =>
      (window as Window & {
        __htmlSpecimenExportedCss?: string
      }).__htmlSpecimenExportedCss ?? ''),
  ).toContain('border-radius: 12px;')
  await expect.poll(async () =>
    page.evaluate(() =>
      (window as Window & {
        __htmlSpecimenExportedCss?: string
      }).__htmlSpecimenExportedCss ?? ''),
  ).toContain('margin: 4px;')
  await expect.poll(async () =>
    page.evaluate(() =>
      (window as Window & {
        __htmlSpecimenExportedCss?: string
      }).__htmlSpecimenExportedCss ?? ''),
  ).toContain('border-color: #f97316;')
  await expect.poll(async () =>
    page.evaluate(() =>
      (window as Window & {
        __htmlSpecimenExportedCss?: string
      }).__htmlSpecimenExportedCss ?? ''),
  ).toContain('padding: 8px 16px;')
  await clickInspectorExport(page, 'HTML')
  await expect.poll(async () =>
    page.evaluate(() =>
      (window as Window & {
        __htmlSpecimenExportedHtml?: string
      }).__htmlSpecimenExportedHtml ?? ''),
  ).toContain('>Launch</button>')

  const previewHtml = await preview.evaluate((host) =>
    host.shadowRoot?.querySelector('[data-preview-surface-root]')
      ?.innerHTML ?? '')

  expect(previewHtml).not.toContain('style=')

  await preview.locator('span#orphan').click()

  const unresolvedTextInput = cssField(page, 'color')
    .locator('input')

  await expect(page.locator('.html-specimen-css-target')).toHaveText(
    'span#orphan',
  )
  await expect(cssField(page, 'color')
    .getByText('No matching rule')).toBeVisible()
  await expect(unresolvedTextInput).toBeDisabled()
})

test('adds missing CSS declarations to the most specific matching rule', async ({
  page,
}) => {
  await page.goto('/')
  await page.evaluate(() => {
    window.addEventListener('html-specimen-css:export', (event) => {
      const targetWindow = window as Window & {
        __htmlSpecimenExportedCss?: string
      }
      const detail = (event as CustomEvent<{ css?: unknown }>).detail

      targetWindow.__htmlSpecimenExportedCss =
        typeof detail.css === 'string' ? detail.css : ''
    })
  })

  const pasteWasHandled = await page.evaluate((text) => {
    const data = new DataTransfer()

    data.setData('text/plain', text)

    const event = new ClipboardEvent('paste', {
      bubbles: true,
      cancelable: true,
      clipboardData: data,
    })

    window.dispatchEvent(event)

    return event.defaultPrevented
  }, JSON.stringify({
    css: [
      '.button {',
      '  color: #334155;',
      '}',
      '.primary {',
      '  background: #2563eb;',
      '}',
      '.button.primary {',
      '  border-radius: 6px;',
      '}',
    ].join('\n'),
    html: [
      '<main>',
      '<button id="primary" class="button primary">Save</button>',
      '<button id="secondary" class="button secondary">Cancel</button>',
      '</main>',
    ].join(''),
  }))

  expect(pasteWasHandled).toBe(true)

  const preview = page.locator('.demo-html-specimen-preview').last()

  await expect(preview).toBeVisible()
  await preview.locator('button#primary').click()

  const fontField = cssField(page, 'font-size')
  const fontInput = fontField.locator('input')

  await expect(fontField.getByText('.button.primary · 1 node')).toBeVisible()
  await expect(fontInput).toBeVisible()
  await fontInput.fill('18px')
  await fontInput.blur()

  await expect.poll(async () =>
    preview.evaluate((host) =>
      host.shadowRoot?.querySelector('style')?.textContent ?? ''),
  ).toContain('font-size: 18px;')
  await expect.poll(async () =>
    preview.locator('button#primary').evaluate((button) =>
      getComputedStyle(button).fontSize),
  ).toBe('18px')
  await expect(fontField.getByText('.button.primary · 1 node')).toBeVisible()

  await clickInspectorExport(page, 'CSS')
  await expect.poll(async () =>
    page.evaluate(() =>
      (window as Window & {
        __htmlSpecimenExportedCss?: string
      }).__htmlSpecimenExportedCss ?? ''),
  ).toContain('font-size: 18px;')

  const previewHtml = await preview.evaluate((host) =>
    host.shadowRoot?.querySelector('[data-preview-surface-root]')
      ?.innerHTML ?? '')

  expect(previewHtml).not.toContain('style=')
})

test('keeps unsupported CSS values out of stylesheet patches', async ({
  page,
}) => {
  await page.goto('/')

  const pasteWasHandled = await page.evaluate((text) => {
    const data = new DataTransfer()

    data.setData('text/plain', text)

    const event = new ClipboardEvent('paste', {
      bubbles: true,
      cancelable: true,
      clipboardData: data,
    })

    window.dispatchEvent(event)

    return event.defaultPrevented
  }, JSON.stringify({
    css: [
      '.button {',
      '  color: #334155;',
      '}',
      '.primary {',
      '  background: #2563eb;',
      '}',
    ].join('\n'),
    html: [
      '<main>',
      '<button id="primary" class="button primary">Save</button>',
      '</main>',
    ].join(''),
  }))

  expect(pasteWasHandled).toBe(true)

  const preview = page.locator('.demo-html-specimen-preview').last()

  await expect(preview).toBeVisible()
  await preview.locator('button#primary').click()

  const fontField = cssField(page, 'font-size')
  const fontInput = fontField.locator('input')

  await expect(fontField.getByText('.primary · 1 node')).toBeVisible()
  await fontInput.fill('not-a-size')
  await fontInput.blur()

  await expect.poll(async () =>
    preview.evaluate((host) =>
      host.shadowRoot?.querySelector('style')?.textContent ?? ''),
  ).not.toContain('font-size: not-a-size;')
  await expect(fontField.getByText('.primary · 1 node')).toBeVisible()
})

test('shows add-rule affected nodes after later overrides are excluded', async ({
  page,
}) => {
  await page.goto('/')

  const pasteWasHandled = await page.evaluate((text) => {
    const data = new DataTransfer()

    data.setData('text/plain', text)

    const event = new ClipboardEvent('paste', {
      bubbles: true,
      cancelable: true,
      clipboardData: data,
    })

    window.dispatchEvent(event)

    return event.defaultPrevented
  }, JSON.stringify({
    css: [
      '.button {',
      '  color: #334155;',
      '}',
      '.danger {',
      '  font-size: 13px;',
      '}',
    ].join('\n'),
    html: [
      '<main>',
      '<button id="primary" class="button">Save</button>',
      '<button id="danger" class="button danger">Delete</button>',
      '</main>',
    ].join(''),
  }))

  expect(pasteWasHandled).toBe(true)

  const preview = page.locator('.demo-html-specimen-preview').last()

  await expect(preview).toBeVisible()
  await preview.locator('button#primary').click()

  const fontField = cssField(page, 'font-size')
  const fontInput = fontField.locator('input')

  await expect(fontField.getByText('.button · 1 node')).toBeVisible()
  await expect(fontInput).toBeVisible()
  await fontInput.fill('18px')
  await fontInput.blur()

  await expect.poll(async () =>
    preview.evaluate((host) =>
      host.shadowRoot?.querySelector('style')?.textContent ?? ''),
  ).toContain('font-size: 18px;')
  await expect.poll(async () =>
    preview.locator('button#primary').evaluate((button) =>
      getComputedStyle(button).fontSize),
  ).toBe('18px')
  await expect.poll(async () =>
    preview.locator('button#danger').evaluate((button) =>
      getComputedStyle(button).fontSize),
  ).toBe('13px')
  await expect(fontField.getByText('.button · 1 node')).toBeVisible()
})

test('patches the later winning declaration without being overwritten', async ({
  page,
}) => {
  await page.goto('/')
  await page.evaluate(() => {
    window.addEventListener('html-specimen-css:export', (event) => {
      const targetWindow = window as Window & {
        __htmlSpecimenExportedCss?: string
      }
      const detail = (event as CustomEvent<{ css?: unknown }>).detail

      targetWindow.__htmlSpecimenExportedCss =
        typeof detail.css === 'string' ? detail.css : ''
    })
  })

  const pasteWasHandled = await page.evaluate((text) => {
    const data = new DataTransfer()

    data.setData('text/plain', text)

    const event = new ClipboardEvent('paste', {
      bubbles: true,
      cancelable: true,
      clipboardData: data,
    })

    window.dispatchEvent(event)

    return event.defaultPrevented
  }, JSON.stringify({
    css: [
      '.primary {',
      '  color: #ffffff;',
      '}',
      '.primary {',
      '  color: #334155;',
      '}',
    ].join('\n'),
    html: [
      '<main>',
      '<button id="primary" class="primary">Save</button>',
      '</main>',
    ].join(''),
  }))

  expect(pasteWasHandled).toBe(true)

  const preview = page.locator('.demo-html-specimen-preview').last()

  await expect(preview).toBeVisible()
  await preview.locator('button#primary').click()

  const textField = cssField(page, 'color')
  const textInput = textField.locator('input')

  await expect(textField.getByText('.primary · 1 node')).toBeVisible()
  await expect(textInput).toHaveValue('#334155')
  await expect.poll(async () =>
    preview.locator('button#primary').evaluate((button) =>
      getComputedStyle(button).color),
  ).toBe('rgb(51, 65, 85)')

  await textInput.fill('#111827')
  await textInput.blur()

  await expect.poll(async () =>
    preview.evaluate((host) => {
      const css = host.shadowRoot?.querySelector('style')?.textContent ?? ''

      return css.includes('color: #ffffff;') &&
        css.includes('color: #111827;') &&
        css.indexOf('color: #ffffff;') < css.indexOf('color: #111827;')
    }),
  ).toBe(true)
  await expect.poll(async () =>
    preview.locator('button#primary').evaluate((button) =>
      getComputedStyle(button).color),
  ).toBe('rgb(17, 24, 39)')
  await clickInspectorExport(page, 'CSS')
  await expect.poll(async () =>
    page.evaluate(() => {
      const css = (window as Window & {
        __htmlSpecimenExportedCss?: string
      }).__htmlSpecimenExportedCss ?? ''

      return css.includes('color: #ffffff;') &&
        css.includes('color: #111827;') &&
        css.indexOf('color: #ffffff;') < css.indexOf('color: #111827;')
    }),
  ).toBe(true)
})

test('reports only nodes whose winning declaration is patched', async ({
  page,
}) => {
  await page.goto('/')

  const pasteWasHandled = await page.evaluate((text) => {
    const data = new DataTransfer()

    data.setData('text/plain', text)

    const event = new ClipboardEvent('paste', {
      bubbles: true,
      cancelable: true,
      clipboardData: data,
    })

    window.dispatchEvent(event)

    return event.defaultPrevented
  }, JSON.stringify({
    css: [
      '.button {',
      '  color: #334155;',
      '}',
      '.danger {',
      '  color: #ef4444;',
      '}',
    ].join('\n'),
    html: [
      '<main>',
      '<button id="primary" class="button primary">Save</button>',
      '<button id="danger" class="button danger">Delete</button>',
      '</main>',
    ].join(''),
  }))

  expect(pasteWasHandled).toBe(true)

  const preview = page.locator('.demo-html-specimen-preview').last()

  await expect(preview).toBeVisible()
  await preview.locator('button#primary').click()

  const textField = cssField(page, 'color')
  const textInput = textField.locator('input')

  await expect(textField.getByText('.button · 1 node')).toBeVisible()
  await expect(textInput).toHaveValue('#334155')
  await textInput.fill('#111827')
  await textInput.blur()

  await expect.poll(async () =>
    preview.locator('button#primary').evaluate((button) =>
      getComputedStyle(button).color),
  ).toBe('rgb(17, 24, 39)')
  await expect.poll(async () =>
    preview.locator('button#danger').evaluate((button) =>
      getComputedStyle(button).color),
  ).toBe('rgb(239, 68, 68)')
})

test('patches important declarations before later non-important declarations', async ({
  page,
}) => {
  await page.goto('/')

  const pasteWasHandled = await page.evaluate((text) => {
    const data = new DataTransfer()

    data.setData('text/plain', text)

    const event = new ClipboardEvent('paste', {
      bubbles: true,
      cancelable: true,
      clipboardData: data,
    })

    window.dispatchEvent(event)

    return event.defaultPrevented
  }, JSON.stringify({
    css: [
      '.primary {',
      '  color: #334155 !important;',
      '}',
      '.primary {',
      '  color: #ffffff;',
      '}',
    ].join('\n'),
    html: [
      '<main>',
      '<button id="primary" class="primary">Save</button>',
      '</main>',
    ].join(''),
  }))

  expect(pasteWasHandled).toBe(true)

  const preview = page.locator('.demo-html-specimen-preview').last()

  await expect(preview).toBeVisible()
  await preview.locator('button#primary').click()

  const textField = cssField(page, 'color')
  const textInput = textField.locator('input')

  await expect(textField.getByText('.primary · 1 node')).toBeVisible()
  await expect(textInput).toHaveValue('#334155')
  await expect.poll(async () =>
    preview.locator('button#primary').evaluate((button) =>
      getComputedStyle(button).color),
  ).toBe('rgb(51, 65, 85)')

  await textInput.fill('#111827')
  await textInput.blur()

  await expect.poll(async () =>
    preview.evaluate((host) => {
      const css = host.shadowRoot?.querySelector('style')?.textContent ?? ''

      return css.includes('color: #111827 !important;') &&
        css.includes('color: #ffffff;') &&
        css.indexOf('color: #111827 !important;') < css.indexOf('color: #ffffff;')
    }),
  ).toBe(true)
  await expect.poll(async () =>
    preview.locator('button#primary').evaluate((button) =>
      getComputedStyle(button).color),
  ).toBe('rgb(17, 24, 39)')
})

test('matches descendant selector rules against DOM ancestry', async ({
  page,
}) => {
  await page.goto('/')
  await page.evaluate(() => {
    window.addEventListener('html-specimen-css:export', (event) => {
      const targetWindow = window as Window & {
        __htmlSpecimenExportedCss?: string
      }
      const detail = (event as CustomEvent<{ css?: unknown }>).detail

      targetWindow.__htmlSpecimenExportedCss =
        typeof detail.css === 'string' ? detail.css : ''
    })
  })

  const pasteWasHandled = await page.evaluate((text) => {
    const data = new DataTransfer()

    data.setData('text/plain', text)

    const event = new ClipboardEvent('paste', {
      bubbles: true,
      cancelable: true,
      clipboardData: data,
    })

    window.dispatchEvent(event)

    return event.defaultPrevented
  }, JSON.stringify({
    css: [
      '.toolbar .primary {',
      '  color: #334155;',
      '}',
    ].join('\n'),
    html: [
      '<main>',
      '<section class="toolbar">',
      '<button id="primary" class="primary">Save</button>',
      '</section>',
      '<button id="loose" class="primary">Loose</button>',
      '</main>',
    ].join(''),
  }))

  expect(pasteWasHandled).toBe(true)

  const preview = page.locator('.demo-html-specimen-preview').last()

  await expect(preview).toBeVisible()
  await preview.locator('button#primary').click()

  const textField = cssField(page, 'color')
  const textInput = textField.locator('input')

  await expect(textField
    .getByText('.toolbar .primary · 1 node'),
  ).toBeVisible()
  await expect(textInput).toHaveValue('#334155')
  await textInput.fill('#111827')
  await textInput.blur()
  await expect.poll(async () =>
    preview.locator('button#primary').evaluate((button) =>
      getComputedStyle(button).color),
  ).toBe('rgb(17, 24, 39)')

  await clickInspectorExport(page, 'CSS')
  await expect.poll(async () =>
    page.evaluate(() =>
      (window as Window & {
        __htmlSpecimenExportedCss?: string
      }).__htmlSpecimenExportedCss ?? ''),
  ).toContain('color: #111827;')

  await preview.locator('button#loose').click()

  const looseTextField = cssField(page, 'color')
  const looseTextInput = looseTextField.locator('input')

  await expect(looseTextField.getByText('No matching rule')).toBeVisible()
  await expect(looseTextInput).toBeDisabled()
})

test('matches adjacent sibling selector rules against DOM paths', async ({
  page,
}) => {
  await page.goto('/')

  const pasteWasHandled = await page.evaluate((text) => {
    const data = new DataTransfer()

    data.setData('text/plain', text)

    const event = new ClipboardEvent('paste', {
      bubbles: true,
      cancelable: true,
      clipboardData: data,
    })

    window.dispatchEvent(event)

    return event.defaultPrevented
  }, JSON.stringify({
    css: [
      '.item + .item {',
      '  color: #334155;',
      '}',
    ].join('\n'),
    html: [
      '<main>',
      '<button id="first" class="item">First</button>',
      '<button id="second" class="item">Second</button>',
      '</main>',
      '<button id="loose" class="item">Loose</button>',
    ].join(''),
  }))

  expect(pasteWasHandled).toBe(true)

  const preview = page.locator('.demo-html-specimen-preview').last()

  await expect(preview).toBeVisible()
  await preview.locator('button#second').click()

  const textField = cssField(page, 'color')
  const textInput = textField.locator('input')

  await expect(textField
    .getByText('.item + .item · 1 node'),
  ).toBeVisible()
  await expect(textInput).toHaveValue('#334155')
  await textInput.fill('#111827')
  await textInput.blur()
  await expect.poll(async () =>
    preview.locator('button#second').evaluate((button) =>
      getComputedStyle(button).color),
  ).toBe('rgb(17, 24, 39)')
  await expect.poll(async () =>
    preview.locator('button#first').evaluate((button) =>
      getComputedStyle(button).color),
  ).not.toBe('rgb(17, 24, 39)')
  await expect.poll(async () =>
    preview.locator('button#loose').evaluate((button) =>
      getComputedStyle(button).color),
  ).not.toBe('rgb(17, 24, 39)')
})

test('matches attribute selector rules against DOM attributes', async ({
  page,
}) => {
  await page.goto('/')

  const pasteWasHandled = await page.evaluate((text) => {
    const data = new DataTransfer()

    data.setData('text/plain', text)

    const event = new ClipboardEvent('paste', {
      bubbles: true,
      cancelable: true,
      clipboardData: data,
    })

    window.dispatchEvent(event)

    return event.defaultPrevented
  }, JSON.stringify({
    css: [
      '.button[data-state="active"] {',
      '  color: #334155;',
      '}',
    ].join('\n'),
    html: [
      '<main>',
      '<button id="active" class="button" data-state="active">Active</button>',
      '<button id="idle" class="button" data-state="idle">Idle</button>',
      '</main>',
    ].join(''),
  }))

  expect(pasteWasHandled).toBe(true)

  const preview = page.locator('.demo-html-specimen-preview').last()

  await expect(preview).toBeVisible()
  await preview.locator('button#active').click()

  const textField = cssField(page, 'color')
  const textInput = textField.locator('input')

  await expect(textField
    .getByText('.button[data-state="active"] · 1 node'),
  ).toBeVisible()
  await expect(textInput).toHaveValue('#334155')
  await textInput.fill('#111827')
  await textInput.blur()
  await expect.poll(async () =>
    preview.locator('button#active').evaluate((button) =>
      getComputedStyle(button).color),
  ).toBe('rgb(17, 24, 39)')

  await preview.locator('button#idle').click()

  const idleTextField = cssField(page, 'color')
  const idleTextInput = idleTextField.locator('input')

  await expect(idleTextField.getByText('No matching rule')).toBeVisible()
  await expect(idleTextInput).toBeDisabled()
})

test('matches space-separated attribute selector rules', async ({
  page,
}) => {
  await page.goto('/')

  const pasteWasHandled = await page.evaluate((text) => {
    const data = new DataTransfer()

    data.setData('text/plain', text)

    const event = new ClipboardEvent('paste', {
      bubbles: true,
      cancelable: true,
      clipboardData: data,
    })

    window.dispatchEvent(event)

    return event.defaultPrevented
  }, JSON.stringify({
    css: [
      '.button[data-state~="active"] {',
      '  color: #334155;',
      '}',
    ].join('\n'),
    html: [
      '<main>',
      '<button id="active" class="button" data-state="primary active">Active</button>',
      '<button id="idle" class="button" data-state="inactive">Idle</button>',
      '</main>',
    ].join(''),
  }))

  expect(pasteWasHandled).toBe(true)

  const preview = page.locator('.demo-html-specimen-preview').last()

  await expect(preview).toBeVisible()
  await preview.locator('button#active').click()

  const textField = cssField(page, 'color')
  const textInput = textField.locator('input')

  await expect(textField
    .getByText('.button[data-state~="active"] · 1 node'),
  ).toBeVisible()
  await expect(textInput).toHaveValue('#334155')
  await textInput.fill('#111827')
  await textInput.blur()
  await expect.poll(async () =>
    preview.locator('button#active').evaluate((button) =>
      getComputedStyle(button).color),
  ).toBe('rgb(17, 24, 39)')

  await preview.locator('button#idle').click()

  const idleTextField = cssField(page, 'color')
  const idleTextInput = idleTextField.locator('input')

  await expect(idleTextField.getByText('No matching rule')).toBeVisible()
  await expect(idleTextInput).toBeDisabled()
})

test('matches escaped utility class selector rules', async ({
  page,
}) => {
  await page.goto('/')

  const pasteWasHandled = await page.evaluate((text) => {
    const data = new DataTransfer()

    data.setData('text/plain', text)

    const event = new ClipboardEvent('paste', {
      bubbles: true,
      cancelable: true,
      clipboardData: data,
    })

    window.dispatchEvent(event)

    return event.defaultPrevented
  }, JSON.stringify({
    css: [
      '.w-\\[12px\\] {',
      '  color: #334155;',
      '}',
    ].join('\n'),
    html: [
      '<main>',
      '<button id="utility" class="w-[12px]">Utility</button>',
      '</main>',
    ].join(''),
  }))

  expect(pasteWasHandled).toBe(true)

  const preview = page.locator('.demo-html-specimen-preview').last()

  await expect(preview).toBeVisible()
  await preview.locator('button#utility').click()

  const textField = cssField(page, 'color')
  const textInput = textField.locator('input')

  await expect(textField
    .getByText('.w-\\[12px\\] · 1 node'),
  ).toBeVisible()
  await expect(textInput).toHaveValue('#334155')
  await textInput.fill('#111827')
  await textInput.blur()
  await expect.poll(async () =>
    preview.locator('button#utility').evaluate((button) =>
      getComputedStyle(button).color),
  ).toBe('rgb(17, 24, 39)')
})

test('matches where pseudo function selector rules', async ({
  page,
}) => {
  await page.goto('/')

  const pasteWasHandled = await page.evaluate((text) => {
    const data = new DataTransfer()

    data.setData('text/plain', text)

    const event = new ClipboardEvent('paste', {
      bubbles: true,
      cancelable: true,
      clipboardData: data,
    })

    window.dispatchEvent(event)

    return event.defaultPrevented
  }, JSON.stringify({
    css: [
      ':where(.button).primary {',
      '  color: #334155;',
      '}',
    ].join('\n'),
    html: [
      '<main>',
      '<button id="primary" class="button primary">Save</button>',
      '<button id="secondary" class="button secondary">Cancel</button>',
      '</main>',
    ].join(''),
  }))

  expect(pasteWasHandled).toBe(true)

  const preview = page.locator('.demo-html-specimen-preview').last()

  await expect(preview).toBeVisible()
  await preview.locator('button#primary').click()

  const textField = cssField(page, 'color')
  const textInput = textField.locator('input')

  await expect(textField
    .getByText(':where(.button).primary · 1 node'),
  ).toBeVisible()
  await expect(textInput).toHaveValue('#334155')
  await textInput.fill('#111827')
  await textInput.blur()
  await expect.poll(async () =>
    preview.locator('button#primary').evaluate((button) =>
      getComputedStyle(button).color),
  ).toBe('rgb(17, 24, 39)')
  await expect.poll(async () =>
    preview.locator('button#secondary').evaluate((button) =>
      getComputedStyle(button).color),
  ).not.toBe('rgb(17, 24, 39)')
})

test('matches not pseudo function selector rules', async ({
  page,
}) => {
  await page.goto('/')

  const pasteWasHandled = await page.evaluate((text) => {
    const data = new DataTransfer()

    data.setData('text/plain', text)

    const event = new ClipboardEvent('paste', {
      bubbles: true,
      cancelable: true,
      clipboardData: data,
    })

    window.dispatchEvent(event)

    return event.defaultPrevented
  }, JSON.stringify({
    css: [
      '.primary:not(.disabled) {',
      '  color: #334155;',
      '}',
    ].join('\n'),
    html: [
      '<main>',
      '<button id="primary" class="primary">Save</button>',
      '<button id="disabled" class="primary disabled">Disabled</button>',
      '</main>',
    ].join(''),
  }))

  expect(pasteWasHandled).toBe(true)

  const preview = page.locator('.demo-html-specimen-preview').last()

  await expect(preview).toBeVisible()
  await preview.locator('button#primary').click()

  const textField = cssField(page, 'color')
  const textInput = textField.locator('input')

  await expect(textField
    .getByText('.primary:not(.disabled) · 1 node'),
  ).toBeVisible()
  await expect(textInput).toHaveValue('#334155')
  await textInput.fill('#111827')
  await textInput.blur()
  await expect.poll(async () =>
    preview.locator('button#primary').evaluate((button) =>
      getComputedStyle(button).color),
  ).toBe('rgb(17, 24, 39)')
  await expect.poll(async () =>
    preview.locator('button#disabled').evaluate((button) =>
      getComputedStyle(button).color),
  ).not.toBe('rgb(17, 24, 39)')
})

test('matches has pseudo function selector rules', async ({
  page,
}) => {
  await page.goto('/')

  const pasteWasHandled = await page.evaluate((text) => {
    const data = new DataTransfer()

    data.setData('text/plain', text)

    const event = new ClipboardEvent('paste', {
      bubbles: true,
      cancelable: true,
      clipboardData: data,
    })

    window.dispatchEvent(event)

    return event.defaultPrevented
  }, JSON.stringify({
    css: [
      '.card:has(.content .badge) {',
      '  color: #334155;',
      '  padding: 12px;',
      '}',
    ].join('\n'),
    html: [
      '<main>',
      '<article id="card" class="card"><section class="content"><span class="badge">New</span></section></article>',
      '<article id="empty" class="card">Empty</article>',
      '</main>',
    ].join(''),
  }))

  expect(pasteWasHandled).toBe(true)

  const preview = page.locator('.demo-html-specimen-preview').last()

  await expect(preview).toBeVisible()
  await preview.locator('article#card').click({ position: { x: 4, y: 4 } })

  const textField = cssField(page, 'color')
  const textInput = textField.locator('input')

  await expect(textField
    .getByText('.card:has(.content .badge) · 1 node'),
  ).toBeVisible()
  await expect(textInput).toHaveValue('#334155')
  await textInput.fill('#111827')
  await textInput.blur()
  await expect.poll(async () =>
    preview.locator('article#card').evaluate((card) =>
      getComputedStyle(card).color),
  ).toBe('rgb(17, 24, 39)')
  await expect.poll(async () =>
    preview.locator('article#empty').evaluate((card) =>
      getComputedStyle(card).color),
  ).not.toBe('rgb(17, 24, 39)')
})

test('keeps commas inside attribute selector values', async ({
  page,
}) => {
  await page.goto('/')

  const pasteWasHandled = await page.evaluate((text) => {
    const data = new DataTransfer()

    data.setData('text/plain', text)

    const event = new ClipboardEvent('paste', {
      bubbles: true,
      cancelable: true,
      clipboardData: data,
    })

    window.dispatchEvent(event)

    return event.defaultPrevented
  }, JSON.stringify({
    css: [
      '.button[data-label="Save, now"] {',
      '  color: #334155;',
      '}',
    ].join('\n'),
    html: [
      '<main>',
      '<button id="save-now" class="button" data-label="Save, now">',
      'Save',
      '</button>',
      '</main>',
    ].join(''),
  }))

  expect(pasteWasHandled).toBe(true)

  const preview = page.locator('.demo-html-specimen-preview').last()

  await expect(preview).toBeVisible()
  await preview.locator('button#save-now').click()

  const textField = cssField(page, 'color')
  const textInput = textField.locator('input')

  await expect(textField
    .getByText('.button[data-label="Save, now"] · 1 node'),
  ).toBeVisible()
  await expect(textInput).toHaveValue('#334155')
  await textInput.fill('#111827')
  await textInput.blur()
  await expect.poll(async () =>
    preview.locator('button#save-now').evaluate((button) =>
      getComputedStyle(button).color),
  ).toBe('rgb(17, 24, 39)')
})

test('ignores braces in CSS comments before rules', async ({
  page,
}) => {
  await page.goto('/')

  const pasteWasHandled = await page.evaluate((text) => {
    const data = new DataTransfer()

    data.setData('text/plain', text)

    const event = new ClipboardEvent('paste', {
      bubbles: true,
      cancelable: true,
      clipboardData: data,
    })

    window.dispatchEvent(event)

    return event.defaultPrevented
  }, JSON.stringify({
    css: [
      '/* section { not a rule } */',
      '.primary {',
      '  color: #334155;',
      '}',
    ].join('\n'),
    html: [
      '<main>',
      '<button id="primary" class="primary">Save</button>',
      '</main>',
    ].join(''),
  }))

  expect(pasteWasHandled).toBe(true)

  const preview = page.locator('.demo-html-specimen-preview').last()

  await expect(preview).toBeVisible()
  await preview.locator('button#primary').click()

  const textField = cssField(page, 'color')
  const textInput = textField.locator('input')

  await expect(textField.getByText('.primary · 1 node')).toBeVisible()
  await expect(textInput).toHaveValue('#334155')
  await textInput.fill('#111827')
  await textInput.blur()
  await expect.poll(async () =>
    preview.locator('button#primary').evaluate((button) =>
      getComputedStyle(button).color),
  ).toBe('rgb(17, 24, 39)')
  await expect.poll(async () =>
    preview.evaluate((host) =>
      host.shadowRoot?.querySelector('style')?.textContent ?? ''),
  ).toContain('/* section { not a rule } */')
})

test('matches enabled pseudo-class selector rules', async ({
  page,
}) => {
  await page.goto('/')

  const pasteWasHandled = await page.evaluate((text) => {
    const data = new DataTransfer()

    data.setData('text/plain', text)

    const event = new ClipboardEvent('paste', {
      bubbles: true,
      cancelable: true,
      clipboardData: data,
    })

    window.dispatchEvent(event)

    return event.defaultPrevented
  }, JSON.stringify({
    css: [
      '.primary:enabled {',
      '  color: #334155;',
      '}',
    ].join('\n'),
    html: [
      '<main>',
      '<button id="primary" class="primary">Save</button>',
      '<button id="disabled" class="primary" disabled>Disabled</button>',
      '</main>',
    ].join(''),
  }))

  expect(pasteWasHandled).toBe(true)

  const preview = page.locator('.demo-html-specimen-preview').last()

  await expect(preview).toBeVisible()
  await preview.locator('button#primary').click()

  const textField = cssField(page, 'color')
  const textInput = textField.locator('input')

  await expect(textField.getByText('.primary:enabled · 1 node'))
    .toBeVisible()
  await expect(textInput).toHaveValue('#334155')
  await textInput.fill('#111827')
  await textInput.blur()
  await expect.poll(async () =>
    preview.evaluate((host) =>
      host.shadowRoot?.querySelector('style')?.textContent ?? ''),
  ).toContain('color: #111827;')
  await expect.poll(async () =>
    preview.locator('button#primary').evaluate((button) =>
      getComputedStyle(button).color),
  ).toBe('rgb(17, 24, 39)')
  await expect.poll(async () =>
    preview.locator('button#disabled').evaluate((button) =>
      getComputedStyle(button).color),
  ).not.toBe('rgb(17, 24, 39)')
})

test('matches fieldset-inherited disabled pseudo-class selector rules', async ({
  page,
}) => {
  await page.goto('/')

  const pasteWasHandled = await page.evaluate((text) => {
    const data = new DataTransfer()

    data.setData('text/plain', text)

    const event = new ClipboardEvent('paste', {
      bubbles: true,
      cancelable: true,
      clipboardData: data,
    })

    window.dispatchEvent(event)

    return event.defaultPrevented
  }, JSON.stringify({
    css: [
      '.primary:disabled {',
      '  color: #334155;',
      '}',
    ].join('\n'),
    html: [
      '<main>',
      '<fieldset disabled>',
      '<button id="primary" class="primary">Save</button>',
      '</fieldset>',
      '<button id="secondary" class="primary">Other</button>',
      '</main>',
    ].join(''),
  }))

  expect(pasteWasHandled).toBe(true)

  const preview = page.locator('.demo-html-specimen-preview').last()

  await expect(preview).toBeVisible()
  await preview.locator('button#primary').evaluate((button) => {
    button.dispatchEvent(new PointerEvent('pointerdown', {
      bubbles: true,
      composed: false,
      pointerId: 1,
    }))
  })

  const textField = cssField(page, 'color')
  const textInput = textField.locator('input')

  await expect(textField.getByText('.primary:disabled · 1 node'))
    .toBeVisible()
  await expect(textInput).toHaveValue('#334155')
  await textInput.fill('#111827')
  await textInput.blur()
  await expect.poll(async () =>
    preview.evaluate((host) =>
      host.shadowRoot?.querySelector('style')?.textContent ?? ''),
  ).toContain('color: #111827;')
  await expect.poll(async () =>
    preview.locator('button#primary').evaluate((button) =>
      getComputedStyle(button).color),
  ).toBe('rgb(17, 24, 39)')
  await expect.poll(async () =>
    preview.locator('button#secondary').evaluate((button) =>
      getComputedStyle(button).color),
  ).not.toBe('rgb(17, 24, 39)')
})

test('matches checked pseudo-class selector rules', async ({
  page,
}) => {
  await page.goto('/')

  const pasteWasHandled = await page.evaluate((text) => {
    const data = new DataTransfer()

    data.setData('text/plain', text)

    const event = new ClipboardEvent('paste', {
      bubbles: true,
      cancelable: true,
      clipboardData: data,
    })

    window.dispatchEvent(event)

    return event.defaultPrevented
  }, JSON.stringify({
    css: [
      '.control:checked + .label {',
      '  color: #334155;',
      '}',
    ].join('\n'),
    html: [
      '<main>',
      '<input id="checked" class="control" type="checkbox" checked>',
      '<span id="checked-label" class="label">On</span>',
      '<input id="unchecked" class="control" type="checkbox">',
      '<span id="unchecked-label" class="label">Off</span>',
      '</main>',
    ].join(''),
  }))

  expect(pasteWasHandled).toBe(true)

  const preview = page.locator('.demo-html-specimen-preview').last()

  await expect(preview).toBeVisible()
  await preview.locator('span#checked-label').click()

  const textField = cssField(page, 'color')
  const textInput = textField.locator('input')

  await expect(textField.getByText('.control:checked + .label · 1 node'))
    .toBeVisible()
  await expect(textInput).toHaveValue('#334155')
  await textInput.fill('#111827')
  await textInput.blur()
  await expect.poll(async () =>
    preview.evaluate((host) =>
      host.shadowRoot?.querySelector('style')?.textContent ?? ''),
  ).toContain('color: #111827;')
  await expect.poll(async () =>
    preview.locator('span#checked-label').evaluate((label) =>
      getComputedStyle(label).color),
  ).toBe('rgb(17, 24, 39)')
  await expect.poll(async () =>
    preview.locator('span#unchecked-label').evaluate((label) =>
      getComputedStyle(label).color),
  ).not.toBe('rgb(17, 24, 39)')
})

test('keeps unsupported pseudo-class selector CSS read-only', async ({
  page,
}) => {
  await page.goto('/')

  const pasteWasHandled = await page.evaluate((text) => {
    const data = new DataTransfer()

    data.setData('text/plain', text)

    const event = new ClipboardEvent('paste', {
      bubbles: true,
      cancelable: true,
      clipboardData: data,
    })

    window.dispatchEvent(event)

    return event.defaultPrevented
  }, JSON.stringify({
    css: [
      '.primary:hover {',
      '  color: #334155;',
      '}',
    ].join('\n'),
    html: [
      '<main>',
      '<button id="primary" class="primary">Save</button>',
      '</main>',
    ].join(''),
  }))

  expect(pasteWasHandled).toBe(true)

  const preview = page.locator('.demo-html-specimen-preview').last()

  await expect(preview).toBeVisible()
  await preview.locator('button#primary').click()

  const textField = cssField(page, 'color')
  const textInput = textField.locator('input')

  await expect(textField.getByText('No matching rule')).toBeVisible()
  await expect(textInput).toBeDisabled()
  await expect.poll(async () =>
    preview.evaluate((host) =>
      host.shadowRoot?.querySelector('style')?.textContent ?? ''),
  ).toContain('.primary:hover')
})

test('matches structural child pseudo class selector rules', async ({
  page,
}) => {
  await page.goto('/')

  const pasteWasHandled = await page.evaluate((text) => {
    const data = new DataTransfer()

    data.setData('text/plain', text)

    const event = new ClipboardEvent('paste', {
      bubbles: true,
      cancelable: true,
      clipboardData: data,
    })

    window.dispatchEvent(event)

    return event.defaultPrevented
  }, JSON.stringify({
    css: [
      '.table-row:first-child {',
      '  color: #334155;',
      '}',
    ].join('\n'),
    html: [
      '<main>',
      '<div id="first" class="table-row">One</div>',
      '<div id="second" class="table-row">Two</div>',
      '</main>',
    ].join(''),
  }))

  expect(pasteWasHandled).toBe(true)

  const preview = page.locator('.demo-html-specimen-preview').last()

  await expect(preview).toBeVisible()
  await preview.locator('div#first').click()

  const textField = cssField(page, 'color')
  const textInput = textField.locator('input')

  await expect(textField
    .getByText('.table-row:first-child · 1 node'),
  ).toBeVisible()
  await expect(textInput).toHaveValue('#334155')
  await textInput.fill('#111827')
  await textInput.blur()
  await expect.poll(async () =>
    preview.locator('div#first').evaluate((row) =>
      getComputedStyle(row).color),
  ).toBe('rgb(17, 24, 39)')
  await expect.poll(async () =>
    preview.locator('div#second').evaluate((row) =>
      getComputedStyle(row).color),
  ).not.toBe('rgb(17, 24, 39)')
})

test('matches nth-child pseudo class selector rules', async ({
  page,
}) => {
  await page.goto('/')

  const pasteWasHandled = await page.evaluate((text) => {
    const data = new DataTransfer()

    data.setData('text/plain', text)

    const event = new ClipboardEvent('paste', {
      bubbles: true,
      cancelable: true,
      clipboardData: data,
    })

    window.dispatchEvent(event)

    return event.defaultPrevented
  }, JSON.stringify({
    css: [
      '.table-row:nth-child(2) {',
      '  color: #334155;',
      '}',
    ].join('\n'),
    html: [
      '<main>',
      '<div id="first" class="table-row">One</div>',
      '<div id="second" class="table-row">Two</div>',
      '</main>',
    ].join(''),
  }))

  expect(pasteWasHandled).toBe(true)

  const preview = page.locator('.demo-html-specimen-preview').last()

  await expect(preview).toBeVisible()
  await preview.locator('div#second').click()

  const textField = cssField(page, 'color')
  const textInput = textField.locator('input')

  await expect(textField
    .getByText('.table-row:nth-child(2) · 1 node'),
  ).toBeVisible()
  await expect(textInput).toHaveValue('#334155')
  await textInput.fill('#111827')
  await textInput.blur()
  await expect.poll(async () =>
    preview.locator('div#second').evaluate((row) =>
      getComputedStyle(row).color),
  ).toBe('rgb(17, 24, 39)')
  await expect.poll(async () =>
    preview.locator('div#first').evaluate((row) =>
      getComputedStyle(row).color),
  ).not.toBe('rgb(17, 24, 39)')
})

test('matches nth-of-type pseudo class selector rules', async ({
  page,
}) => {
  await page.goto('/')

  const pasteWasHandled = await page.evaluate((text) => {
    const data = new DataTransfer()

    data.setData('text/plain', text)

    const event = new ClipboardEvent('paste', {
      bubbles: true,
      cancelable: true,
      clipboardData: data,
    })

    window.dispatchEvent(event)

    return event.defaultPrevented
  }, JSON.stringify({
    css: [
      '.table-row:nth-of-type(2) {',
      '  color: #334155;',
      '}',
    ].join('\n'),
    html: [
      '<main>',
      '<div id="first" class="table-row">One</div>',
      '<button id="gap" class="gap">Gap</button>',
      '<div id="second" class="table-row">Two</div>',
      '</main>',
    ].join(''),
  }))

  expect(pasteWasHandled).toBe(true)

  const preview = page.locator('.demo-html-specimen-preview').last()

  await expect(preview).toBeVisible()
  await preview.locator('div#second').click()

  const textField = cssField(page, 'color')
  const textInput = textField.locator('input')

  await expect(textField
    .getByText('.table-row:nth-of-type(2) · 1 node'),
  ).toBeVisible()
  await expect(textInput).toHaveValue('#334155')
  await textInput.fill('#111827')
  await textInput.blur()
  await expect.poll(async () =>
    preview.locator('div#second').evaluate((row) =>
      getComputedStyle(row).color),
  ).toBe('rgb(17, 24, 39)')
  await expect.poll(async () =>
    preview.locator('div#first').evaluate((row) =>
      getComputedStyle(row).color),
  ).not.toBe('rgb(17, 24, 39)')
})

test('edits safe token-backed preview CSS through custom properties', async ({
  page,
}) => {
  await page.goto('/')

  const pasteWasHandled = await page.evaluate((text) => {
    const data = new DataTransfer()

    data.setData('text/plain', text)

    const event = new ClipboardEvent('paste', {
      bubbles: true,
      cancelable: true,
      clipboardData: data,
    })

    window.dispatchEvent(event)

    return event.defaultPrevented
  }, JSON.stringify({
    css: [
      ':root, html {',
      '  --brand: #2563eb;',
      '  --control-font: 700 14px/1 system-ui;',
      '}',
      '.primary {',
      '  color: #ffffff;',
      '  background: var(--brand, #0f172a);',
      '  font: var(--control-font);',
      '}',
    ].join('\n'),
    html: [
      '<main>',
      '<button id="primary" class="primary">Save</button>',
      '</main>',
    ].join(''),
  }))

  expect(pasteWasHandled).toBe(true)

  const preview = page.locator('.demo-html-specimen-preview').last()

  await expect(preview).toBeVisible()
  await preview.locator('button#primary').click()

  const backgroundField = cssField(page, 'background-color')
  const backgroundInput = backgroundField.locator('input')
  const fontField = cssField(page, 'font-size')
  const fontInput = fontField.locator('input')

  await expect(backgroundField.getByText('token :root, html · 1 node')).toBeVisible()
  await expect(backgroundInput).toHaveValue('#2563eb')
  await backgroundInput.fill('#111827')
  await backgroundInput.blur()
  await expect.poll(async () =>
    preview.locator('button#primary').evaluate((button) =>
      getComputedStyle(button).backgroundColor),
  ).toBe('rgb(17, 24, 39)')
  await expect(fontField.getByText('token .primary · 1 node')).toBeVisible()
  await expect(fontInput).toHaveValue('var(--control-font)')
  await expect(fontInput).toBeDisabled()
  await expect.poll(async () =>
    preview.evaluate((host) =>
      host.shadowRoot?.querySelector('style')?.textContent ?? ''),
  ).toContain('--brand: #111827;')
  await expect.poll(async () =>
    preview.evaluate((host) =>
      host.shadowRoot?.querySelector('style')?.textContent ?? ''),
  ).toContain('background: var(--brand, #0f172a);')
  await expect.poll(async () =>
    preview.evaluate((host) =>
      host.shadowRoot?.querySelector('style')?.textContent ?? ''),
  ).toContain('font: var(--control-font);')
})

test('skips token patches when requested fill already matches computed style', async ({
  page,
}) => {
  await page.goto('/')

  const pasteWasHandled = await page.evaluate((text) => {
    const data = new DataTransfer()

    data.setData('text/plain', text)

    const event = new ClipboardEvent('paste', {
      bubbles: true,
      cancelable: true,
      clipboardData: data,
    })

    window.dispatchEvent(event)

    return event.defaultPrevented
  }, JSON.stringify({
    css: [
      ':root {',
      '  --brand: #2563eb;',
      '}',
      '.primary {',
      '  background: var(--brand);',
      '}',
    ].join('\n'),
    html: [
      '<main>',
      '<button id="primary" class="primary">Save</button>',
      '</main>',
    ].join(''),
  }))

  expect(pasteWasHandled).toBe(true)

  const preview = page.locator('.demo-html-specimen-preview').last()

  await expect(preview).toBeVisible()
  await preview.locator('button#primary').click()

  const backgroundField = cssField(page, 'background-color')
  const backgroundInput = backgroundField.locator('input')
  const computedBackground = await preview
    .locator('button#primary')
    .evaluate((button) => getComputedStyle(button).backgroundColor)

  await expect(backgroundField.getByText('token :root · 1 node')).toBeVisible()
  await expect(backgroundInput).toHaveValue('#2563eb')
  expect(computedBackground).toBe('rgb(37, 99, 235)')
  await backgroundInput.fill(computedBackground)
  await backgroundInput.blur()

  await expect.poll(async () =>
    preview.evaluate((host) =>
      host.shadowRoot?.querySelector('style')?.textContent ?? ''),
  ).toContain('--brand: #2563eb;')
  await expect.poll(async () =>
    preview.evaluate((host) =>
      host.shadowRoot?.querySelector('style')?.textContent ?? ''),
  ).not.toContain('--brand: rgb(37, 99, 235);')
  await expect.poll(async () =>
    preview.evaluate((host) =>
      host.shadowRoot?.querySelector('style')?.textContent ?? ''),
  ).toContain('background: var(--brand);')
})

test('edits inherited token-backed preview CSS through ancestor custom properties', async ({
  page,
}) => {
  await page.goto('/')

  const pasteWasHandled = await page.evaluate((text) => {
    const data = new DataTransfer()

    data.setData('text/plain', text)

    const event = new ClipboardEvent('paste', {
      bubbles: true,
      cancelable: true,
      clipboardData: data,
    })

    window.dispatchEvent(event)

    return event.defaultPrevented
  }, JSON.stringify({
    css: [
      '.theme {',
      '  --brand: #2563eb;',
      '}',
      '.primary {',
      '  color: #ffffff;',
      '  background: var(--brand);',
      '}',
    ].join('\n'),
    html: [
      '<main class="theme">',
      '<button id="primary" class="primary">Save</button>',
      '</main>',
    ].join(''),
  }))

  expect(pasteWasHandled).toBe(true)

  const preview = page.locator('.demo-html-specimen-preview').last()

  await expect(preview).toBeVisible()
  await preview.locator('button#primary').click()

  const backgroundField = cssField(page, 'background-color')
  const backgroundInput = backgroundField.locator('input')

  await expect(backgroundField.getByText('token .theme · 1 node')).toBeVisible()
  await expect(backgroundInput).toHaveValue('#2563eb')
  await backgroundInput.fill('#111827')
  await backgroundInput.blur()
  await expect.poll(async () =>
    preview.locator('button#primary').evaluate((button) =>
      getComputedStyle(button).backgroundColor),
  ).toBe('rgb(17, 24, 39)')
  await expect.poll(async () =>
    preview.evaluate((host) =>
      host.shadowRoot?.querySelector('style')?.textContent ?? ''),
  ).toContain('--brand: #111827;')
  await expect.poll(async () =>
    preview.evaluate((host) =>
      host.shadowRoot?.querySelector('style')?.textContent ?? ''),
  ).toContain('background: var(--brand);')
})

test('keeps complex background shorthand fill edits read-only', async ({
  page,
}) => {
  await page.goto('/')

  const pasteWasHandled = await page.evaluate((text) => {
    const data = new DataTransfer()

    data.setData('text/plain', text)

    const event = new ClipboardEvent('paste', {
      bubbles: true,
      cancelable: true,
      clipboardData: data,
    })

    window.dispatchEvent(event)

    return event.defaultPrevented
  }, JSON.stringify({
    css: [
      '.primary {',
      '  color: #ffffff;',
      '  background: linear-gradient(#ffffff, #f8fafc), #2563eb;',
      '}',
    ].join('\n'),
    html: [
      '<main>',
      '<button id="primary" class="primary">Save</button>',
      '</main>',
    ].join(''),
  }))

  expect(pasteWasHandled).toBe(true)

  const preview = page.locator('.demo-html-specimen-preview').last()

  await expect(preview).toBeVisible()
  await preview.locator('button#primary').click()

  const backgroundField = cssField(page, 'background-color')
  const backgroundInput = backgroundField.locator('input')

  await expect(backgroundField
    .getByText('conflict .primary · 1 node'),
  ).toBeVisible()
  await expect(backgroundInput).toBeDisabled()
  await expect.poll(async () =>
    preview.evaluate((host) =>
      host.shadowRoot?.querySelector('style')?.textContent ?? ''),
  ).toContain('linear-gradient(#ffffff, #f8fafc), #2563eb')
})

test('allows longhand edits after token-backed shorthand is overridden', async ({
  page,
}) => {
  await page.goto('/')

  const pasteWasHandled = await page.evaluate((text) => {
    const data = new DataTransfer()

    data.setData('text/plain', text)

    const event = new ClipboardEvent('paste', {
      bubbles: true,
      cancelable: true,
      clipboardData: data,
    })

    window.dispatchEvent(event)

    return event.defaultPrevented
  }, JSON.stringify({
    css: [
      ':root {',
      '  --control-font: 700 14px/1 system-ui;',
      '}',
      '.button {',
      '  font: var(--control-font);',
      '}',
      '.danger {',
      '  font-size: 13px;',
      '}',
    ].join('\n'),
    html: [
      '<main>',
      '<button id="primary" class="button">Save</button>',
      '<button id="danger" class="button danger">Delete</button>',
      '</main>',
    ].join(''),
  }))

  expect(pasteWasHandled).toBe(true)

  const preview = page.locator('.demo-html-specimen-preview').last()

  await expect(preview).toBeVisible()
  await preview.locator('button#danger').click()
  const primaryFontSize = await preview.locator('button#primary')
    .evaluate((button) => getComputedStyle(button).fontSize)

  const fontField = cssField(page, 'font-size')
  const fontInput = fontField.locator('input')

  await expect(fontField.getByText('.danger · 1 node')).toBeVisible()
  await expect(fontInput).toHaveValue('13px')
  await expect(fontInput).toBeEnabled()
  await fontInput.fill('18px')
  await fontInput.blur()

  await expect.poll(async () =>
    preview.locator('button#danger').evaluate((button) =>
      getComputedStyle(button).fontSize),
  ).toBe('18px')
  await expect.poll(async () =>
    preview.locator('button#primary').evaluate((button) =>
      getComputedStyle(button).fontSize),
  ).toBe(primaryFontSize)
})

test('keeps shorthand controls read-only when longhand declarations exist', async ({
  page,
}) => {
  await page.goto('/')

  const pasteWasHandled = await page.evaluate((text) => {
    const data = new DataTransfer()

    data.setData('text/plain', text)

    const event = new ClipboardEvent('paste', {
      bubbles: true,
      cancelable: true,
      clipboardData: data,
    })

    window.dispatchEvent(event)

    return event.defaultPrevented
  }, JSON.stringify({
    css: [
      '.primary {',
      '  color: #ffffff;',
      '  margin-top: 4px;',
      '}',
    ].join('\n'),
    html: [
      '<main>',
      '<button id="primary" class="primary">Save</button>',
      '</main>',
    ].join(''),
  }))

  expect(pasteWasHandled).toBe(true)

  const preview = page.locator('.demo-html-specimen-preview').last()

  await expect(preview).toBeVisible()
  await preview.locator('button#primary').click()

  const marginField = cssField(page, 'margin')
  const marginInput = marginField.locator('input')

  await expect(marginField.getByText('conflict .primary · 1 node')).toBeVisible()
  await expect(marginInput).toBeDisabled()
  await expect.poll(async () =>
    preview.evaluate((host) =>
      host.shadowRoot?.querySelector('style')?.textContent ?? ''),
  ).toContain('margin-top: 4px;')
})

test('edits a shorthand that already beats earlier longhands', async ({
  page,
}) => {
  await page.goto('/')

  const pasteWasHandled = await page.evaluate((text) => {
    const data = new DataTransfer()

    data.setData('text/plain', text)

    const event = new ClipboardEvent('paste', {
      bubbles: true,
      cancelable: true,
      clipboardData: data,
    })

    window.dispatchEvent(event)

    return event.defaultPrevented
  }, JSON.stringify({
    css: [
      '.primary {',
      '  color: #ffffff;',
      '  margin-top: 4px;',
      '  margin: 8px;',
      '}',
    ].join('\n'),
    html: [
      '<main>',
      '<button id="primary" class="primary">Save</button>',
      '</main>',
    ].join(''),
  }))

  expect(pasteWasHandled).toBe(true)

  const preview = page.locator('.demo-html-specimen-preview').last()

  await expect(preview).toBeVisible()
  await preview.locator('button#primary').click()

  const marginField = cssField(page, 'margin')
  const marginInput = marginField.locator('input')

  await expect(marginField.getByText('.primary · 1 node')).toBeVisible()
  await expect(marginInput).toHaveValue('8px')
  await expect(marginInput).toBeEnabled()
  await marginInput.fill('12px')
  await marginInput.blur()

  await expect.poll(async () =>
    preview.evaluate((host) =>
      host.shadowRoot?.querySelector('style')?.textContent ?? ''),
  ).toContain('margin: 12px;')
  await expect.poll(async () =>
    preview.locator('button#primary').evaluate((button) =>
      getComputedStyle(button).marginTop),
  ).toBe('12px')
})

test('edits scoped at-rule CSS in the inspector', async ({
  page,
}) => {
  await page.goto('/')

  const pasteWasHandled = await page.evaluate((text) => {
    const data = new DataTransfer()

    data.setData('text/plain', text)

    const event = new ClipboardEvent('paste', {
      bubbles: true,
      cancelable: true,
      clipboardData: data,
    })

    window.dispatchEvent(event)

    return event.defaultPrevented
  }, JSON.stringify({
    css: [
      '@media (min-width: 1px) {',
      '  .button {',
      '    color: #ffffff;',
      '  }',
      '  .danger {',
      '    color: #ef4444;',
      '  }',
      '}',
    ].join('\n'),
    html: [
      '<main>',
      '<button id="primary" class="button">Save</button>',
      '<button id="danger" class="button danger">Delete</button>',
      '</main>',
    ].join(''),
  }))

  expect(pasteWasHandled).toBe(true)

  const preview = page.locator('.demo-html-specimen-preview').last()

  await expect(preview).toBeVisible()
  await preview.locator('button#primary').click()

  const textField = cssField(page, 'color')
  const textInput = textField.locator('input')

  await expect(textField
    .getByText('@media (min-width: 1px) / .button · 1 node'),
  ).toBeVisible()
  await expect(textInput).toHaveValue('#ffffff')
  await expect(textInput).toBeEnabled()
  await textInput.fill('#111827')
  await textInput.blur()
  await expect.poll(async () =>
    preview.evaluate((host) =>
      host.shadowRoot?.querySelector('style')?.textContent ?? ''),
  ).toContain('color: #111827;')
  await expect.poll(async () =>
    preview.locator('button#primary').evaluate((button) =>
      getComputedStyle(button).color),
  ).toBe('rgb(17, 24, 39)')
  await expect.poll(async () =>
    preview.locator('button#danger').evaluate((button) =>
      getComputedStyle(button).color),
  ).toBe('rgb(239, 68, 68)')
  await expect.poll(async () =>
    preview.evaluate((host) =>
      host.shadowRoot?.querySelector('style')?.textContent ?? ''),
  ).toContain('@media (min-width: 1px)')
})

test('ignores inactive media rules in the inspector', async ({
  page,
}) => {
  await page.goto('/')

  const pasteWasHandled = await page.evaluate((text) => {
    const data = new DataTransfer()

    data.setData('text/plain', text)

    const event = new ClipboardEvent('paste', {
      bubbles: true,
      cancelable: true,
      clipboardData: data,
    })

    window.dispatchEvent(event)

    return event.defaultPrevented
  }, JSON.stringify({
    css: [
      '@media (min-width: 99999px) {',
      '  .primary {',
      '    color: #ffffff;',
      '  }',
      '}',
      '.primary {',
      '  color: #334155;',
      '}',
    ].join('\n'),
    html: [
      '<main>',
      '<button id="primary" class="primary">Save</button>',
      '</main>',
    ].join(''),
  }))

  expect(pasteWasHandled).toBe(true)

  const preview = page.locator('.demo-html-specimen-preview').last()

  await expect(preview).toBeVisible()
  await preview.locator('button#primary').click()

  const textField = cssField(page, 'color')
  const textInput = textField.locator('input')

  await expect(textField.getByText('.primary · 1 node')).toBeVisible()
  await expect(textField.getByText('@media')).not.toBeVisible()
  await expect(textInput).toHaveValue('#334155')
  await textInput.fill('#111827')
  await textInput.blur()
  await expect.poll(async () =>
    preview.evaluate((host) =>
      host.shadowRoot?.querySelector('style')?.textContent ?? ''),
  ).toContain('color: #111827;')
  await expect.poll(async () =>
    preview.evaluate((host) =>
      host.shadowRoot?.querySelector('style')?.textContent ?? ''),
  ).toContain('color: #ffffff;')
  await expect.poll(async () =>
    preview.locator('button#primary').evaluate((button) =>
      getComputedStyle(button).color),
  ).toBe('rgb(17, 24, 39)')
})

test('ignores inactive media range syntax rules in the inspector', async ({
  page,
}) => {
  await page.goto('/')

  const pasteWasHandled = await page.evaluate((text) => {
    const data = new DataTransfer()

    data.setData('text/plain', text)

    const event = new ClipboardEvent('paste', {
      bubbles: true,
      cancelable: true,
      clipboardData: data,
    })

    window.dispatchEvent(event)

    return event.defaultPrevented
  }, JSON.stringify({
    css: [
      '@media (width >= 99999px) {',
      '  .primary {',
      '    color: #ffffff;',
      '  }',
      '}',
      '.primary {',
      '  color: #334155;',
      '}',
    ].join('\n'),
    html: [
      '<main>',
      '<button id="primary" class="primary">Save</button>',
      '</main>',
    ].join(''),
  }))

  expect(pasteWasHandled).toBe(true)

  const preview = page.locator('.demo-html-specimen-preview').last()

  await expect(preview).toBeVisible()
  await preview.locator('button#primary').click()

  const textField = cssField(page, 'color')
  const textInput = textField.locator('input')

  await expect(textField.getByText('.primary · 1 node')).toBeVisible()
  await expect(textField.getByText('@media')).not.toBeVisible()
  await expect(textInput).toHaveValue('#334155')
  await textInput.fill('#111827')
  await textInput.blur()
  await expect.poll(async () =>
    preview.evaluate((host) =>
      host.shadowRoot?.querySelector('style')?.textContent ?? ''),
  ).toContain('color: #111827;')
  await expect.poll(async () =>
    preview.evaluate((host) =>
      host.shadowRoot?.querySelector('style')?.textContent ?? ''),
  ).toContain('color: #ffffff;')
  await expect.poll(async () =>
    preview.locator('button#primary').evaluate((button) =>
      getComputedStyle(button).color),
  ).toBe('rgb(17, 24, 39)')
})

test('ignores unsupported container rules in the inspector', async ({
  page,
}) => {
  await page.goto('/')

  const pasteWasHandled = await page.evaluate((text) => {
    const data = new DataTransfer()

    data.setData('text/plain', text)

    const event = new ClipboardEvent('paste', {
      bubbles: true,
      cancelable: true,
      clipboardData: data,
    })

    window.dispatchEvent(event)

    return event.defaultPrevented
  }, JSON.stringify({
    css: [
      '.primary {',
      '  color: #334155;',
      '}',
      '@container (min-width: 99999px) {',
      '  .primary {',
      '    color: #ffffff;',
      '  }',
      '}',
    ].join('\n'),
    html: [
      '<main>',
      '<button id="primary" class="primary">Save</button>',
      '</main>',
    ].join(''),
  }))

  expect(pasteWasHandled).toBe(true)

  const preview = page.locator('.demo-html-specimen-preview').last()

  await expect(preview).toBeVisible()
  await preview.locator('button#primary').click()

  const textField = cssField(page, 'color')
  const textInput = textField.locator('input')

  await expect(textField.getByText('.primary · 1 node')).toBeVisible()
  await expect(textField.getByText('@container')).not.toBeVisible()
  await expect(textInput).toHaveValue('#334155')
  await textInput.fill('#111827')
  await textInput.blur()
  await expect.poll(async () =>
    preview.evaluate((host) =>
      host.shadowRoot?.querySelector('style')?.textContent ?? ''),
  ).toContain(`.primary {
  color: #111827;
}`)
  await expect.poll(async () =>
    preview.evaluate((host) =>
      host.shadowRoot?.querySelector('style')?.textContent ?? ''),
  ).toContain('color: #ffffff;')
  await expect.poll(async () =>
    preview.locator('button#primary').evaluate((button) =>
      getComputedStyle(button).color),
  ).toBe('rgb(17, 24, 39)')
})

test('ignores unsupported scope rules in the inspector', async ({
  page,
}) => {
  await page.goto('/')

  const pasteWasHandled = await page.evaluate((text) => {
    const data = new DataTransfer()

    data.setData('text/plain', text)

    const event = new ClipboardEvent('paste', {
      bubbles: true,
      cancelable: true,
      clipboardData: data,
    })

    window.dispatchEvent(event)

    return event.defaultPrevented
  }, JSON.stringify({
    css: [
      '.primary {',
      '  color: #334155;',
      '}',
      '@scope (.dialog) {',
      '  .primary {',
      '    color: #ffffff;',
      '  }',
      '}',
    ].join('\n'),
    html: [
      '<main>',
      '<button id="primary" class="primary">Save</button>',
      '</main>',
    ].join(''),
  }))

  expect(pasteWasHandled).toBe(true)

  const preview = page.locator('.demo-html-specimen-preview').last()

  await expect(preview).toBeVisible()
  await preview.locator('button#primary').click()

  const textField = cssField(page, 'color')
  const textInput = textField.locator('input')

  await expect(textField.getByText('.primary · 1 node')).toBeVisible()
  await expect(textField.getByText('@scope')).not.toBeVisible()
  await expect(textInput).toHaveValue('#334155')
  await textInput.fill('#111827')
  await textInput.blur()
  await expect.poll(async () =>
    preview.evaluate((host) =>
      host.shadowRoot?.querySelector('style')?.textContent ?? ''),
  ).toContain(`.primary {
  color: #111827;
}`)
  await expect.poll(async () =>
    preview.evaluate((host) =>
      host.shadowRoot?.querySelector('style')?.textContent ?? ''),
  ).toContain('color: #ffffff;')
  await expect.poll(async () =>
    preview.locator('button#primary').evaluate((button) =>
      getComputedStyle(button).color),
  ).toBe('rgb(17, 24, 39)')
})

test('ignores unsupported nested at-rules in the inspector', async ({
  page,
}) => {
  await page.goto('/')

  const pasteWasHandled = await page.evaluate((text) => {
    const data = new DataTransfer()

    data.setData('text/plain', text)

    const event = new ClipboardEvent('paste', {
      bubbles: true,
      cancelable: true,
      clipboardData: data,
    })

    window.dispatchEvent(event)

    return event.defaultPrevented
  }, JSON.stringify({
    css: [
      '.primary {',
      '  color: #334155;',
      '}',
      '@unknown preview-editor {',
      '  .primary {',
      '    color: #ffffff;',
      '  }',
      '}',
    ].join('\n'),
    html: [
      '<main>',
      '<button id="primary" class="primary">Save</button>',
      '</main>',
    ].join(''),
  }))

  expect(pasteWasHandled).toBe(true)

  const preview = page.locator('.demo-html-specimen-preview').last()

  await expect(preview).toBeVisible()
  await preview.locator('button#primary').click()

  const textField = cssField(page, 'color')
  const textInput = textField.locator('input')

  await expect(textField.getByText('.primary · 1 node')).toBeVisible()
  await expect(textField.getByText('@unknown')).not.toBeVisible()
  await expect(textInput).toHaveValue('#334155')
  await textInput.fill('#111827')
  await textInput.blur()
  await expect.poll(async () =>
    preview.evaluate((host) =>
      host.shadowRoot?.querySelector('style')?.textContent ?? ''),
  ).toContain(`.primary {
  color: #111827;
}`)
  await expect.poll(async () =>
    preview.evaluate((host) =>
      host.shadowRoot?.querySelector('style')?.textContent ?? ''),
  ).toContain('color: #ffffff;')
  await expect.poll(async () =>
    preview.locator('button#primary').evaluate((button) =>
      getComputedStyle(button).color),
  ).toBe('rgb(17, 24, 39)')
})

test('ignores inactive supports rules in the inspector', async ({
  page,
}) => {
  await page.goto('/')

  const pasteWasHandled = await page.evaluate((text) => {
    const data = new DataTransfer()

    data.setData('text/plain', text)

    const event = new ClipboardEvent('paste', {
      bubbles: true,
      cancelable: true,
      clipboardData: data,
    })

    window.dispatchEvent(event)

    return event.defaultPrevented
  }, JSON.stringify({
    css: [
      '.primary {',
      '  color: #334155;',
      '}',
      '@supports not (display: block) {',
      '  .primary {',
      '    color: #ffffff;',
      '  }',
      '}',
    ].join('\n'),
    html: [
      '<main>',
      '<button id="primary" class="primary">Save</button>',
      '</main>',
    ].join(''),
  }))

  expect(pasteWasHandled).toBe(true)

  const preview = page.locator('.demo-html-specimen-preview').last()

  await expect(preview).toBeVisible()
  await preview.locator('button#primary').click()

  const textField = cssField(page, 'color')
  const textInput = textField.locator('input')

  await expect(textField.getByText('.primary · 1 node')).toBeVisible()
  await expect(textField.getByText('@supports')).not.toBeVisible()
  await expect(textInput).toHaveValue('#334155')
  await textInput.fill('#111827')
  await textInput.blur()
  await expect.poll(async () =>
    preview.evaluate((host) =>
      host.shadowRoot?.querySelector('style')?.textContent ?? ''),
  ).toContain('color: #111827;')
  await expect.poll(async () =>
    preview.evaluate((host) =>
      host.shadowRoot?.querySelector('style')?.textContent ?? ''),
  ).toContain('color: #ffffff;')
  await expect.poll(async () =>
    preview.locator('button#primary').evaluate((button) =>
      getComputedStyle(button).color),
  ).toBe('rgb(17, 24, 39)')
})

test('ignores unknown supports rules in the inspector', async ({
  page,
}) => {
  await page.goto('/')

  const pasteWasHandled = await page.evaluate((text) => {
    const data = new DataTransfer()

    data.setData('text/plain', text)

    const event = new ClipboardEvent('paste', {
      bubbles: true,
      cancelable: true,
      clipboardData: data,
    })

    window.dispatchEvent(event)

    return event.defaultPrevented
  }, JSON.stringify({
    css: [
      '.primary {',
      '  color: #334155;',
      '}',
      '@supports (unknown-preview-editor-property: enabled) {',
      '  .primary {',
      '    color: #ffffff;',
      '  }',
      '}',
    ].join('\n'),
    html: [
      '<main>',
      '<button id="primary" class="primary">Save</button>',
      '</main>',
    ].join(''),
  }))

  expect(pasteWasHandled).toBe(true)

  const preview = page.locator('.demo-html-specimen-preview').last()

  await expect(preview).toBeVisible()
  await preview.locator('button#primary').click()

  const textField = cssField(page, 'color')
  const textInput = textField.locator('input')

  await expect(textField.getByText('.primary · 1 node')).toBeVisible()
  await expect(textField.getByText('@supports')).not.toBeVisible()
  await expect(textInput).toHaveValue('#334155')
  await textInput.fill('#111827')
  await textInput.blur()
  await expect.poll(async () =>
    preview.evaluate((host) =>
      host.shadowRoot?.querySelector('style')?.textContent ?? ''),
  ).toContain('color: #111827;')
  await expect.poll(async () =>
    preview.evaluate((host) =>
      host.shadowRoot?.querySelector('style')?.textContent ?? ''),
  ).toContain('color: #ffffff;')
  await expect.poll(async () =>
    preview.locator('button#primary').evaluate((button) =>
      getComputedStyle(button).color),
  ).toBe('rgb(17, 24, 39)')
})

test('keeps unlayered declarations above later normal layer rules', async ({
  page,
}) => {
  await page.goto('/')

  const pasteWasHandled = await page.evaluate((text) => {
    const data = new DataTransfer()

    data.setData('text/plain', text)

    const event = new ClipboardEvent('paste', {
      bubbles: true,
      cancelable: true,
      clipboardData: data,
    })

    window.dispatchEvent(event)

    return event.defaultPrevented
  }, JSON.stringify({
    css: [
      '.primary {',
      '  color: #334155;',
      '}',
      '@layer components {',
      '  .primary {',
      '    color: #ef4444;',
      '  }',
      '}',
    ].join('\n'),
    html: [
      '<main>',
      '<button id="primary" class="primary">Save</button>',
      '</main>',
    ].join(''),
  }))

  expect(pasteWasHandled).toBe(true)

  const preview = page.locator('.demo-html-specimen-preview').last()

  await expect(preview).toBeVisible()
  await preview.locator('button#primary').click()

  const textField = cssField(page, 'color')
  const textInput = textField.locator('input')

  await expect(textField.getByText('.primary · 1 node')).toBeVisible()
  await expect(textField.getByText('@layer components'))
    .not.toBeVisible()
  await expect(textInput).toHaveValue('#334155')
  await textInput.fill('#111827')
  await textInput.blur()
  await expect.poll(async () =>
    preview.evaluate((host) =>
      host.shadowRoot?.querySelector('style')?.textContent ?? ''),
  ).toContain(`.primary {
  color: #111827;
}`)
  await expect.poll(async () =>
    preview.evaluate((host) =>
      host.shadowRoot?.querySelector('style')?.textContent ?? ''),
  ).toContain(`@layer components {
  .primary {
    color: #ef4444;
  }
}`)
  await expect.poll(async () =>
    preview.locator('button#primary').evaluate((button) =>
      getComputedStyle(button).color),
  ).toBe('rgb(17, 24, 39)')
})

test('keeps inline style winners read-only in the inspector', async ({
  page,
}) => {
  await page.goto('/')

  const pasteWasHandled = await page.evaluate((text) => {
    const data = new DataTransfer()

    data.setData('text/plain', text)

    const event = new ClipboardEvent('paste', {
      bubbles: true,
      cancelable: true,
      clipboardData: data,
    })

    window.dispatchEvent(event)

    return event.defaultPrevented
  }, JSON.stringify({
    css: [
      '.primary {',
      '  color: #334155;',
      '}',
    ].join('\n'),
    html: [
      '<main>',
      '<button id="primary" class="primary" style="color: #ef4444;">Save</button>',
      '</main>',
    ].join(''),
  }))

  expect(pasteWasHandled).toBe(true)

  const preview = page.locator('.demo-html-specimen-preview').last()

  await expect(preview).toBeVisible()
  await preview.locator('button#primary').click()

  const textField = cssField(page, 'color')
  const textInput = textField.locator('input')

  await expect(textField.getByText('inline style · 1 node')).toBeVisible()
  await expect(textInput).toHaveValue(/#ef4444|rgb\(239, 68, 68\)/)
  await expect(textInput).toBeDisabled()
  await expect.poll(async () =>
    preview.locator('button#primary').evaluate((button) =>
      getComputedStyle(button).color),
  ).toBe('rgb(239, 68, 68)')
  await expect.poll(async () =>
    preview.evaluate((host) =>
      host.shadowRoot?.querySelector('style')?.textContent ?? ''),
  ).toContain('color: #334155;')
})

test('keeps important inline style winners read-only in the inspector', async ({
  page,
}) => {
  await page.goto('/')

  const pasteWasHandled = await page.evaluate((text) => {
    const data = new DataTransfer()

    data.setData('text/plain', text)

    const event = new ClipboardEvent('paste', {
      bubbles: true,
      cancelable: true,
      clipboardData: data,
    })

    window.dispatchEvent(event)

    return event.defaultPrevented
  }, JSON.stringify({
    css: [
      '.primary {',
      '  color: #334155 !important;',
      '}',
    ].join('\n'),
    html: [
      '<main>',
      '<button id="primary" class="primary" style="color: #ef4444 !important; color: #334155;">Save</button>',
      '</main>',
    ].join(''),
  }))

  expect(pasteWasHandled).toBe(true)

  const preview = page.locator('.demo-html-specimen-preview').last()

  await expect(preview).toBeVisible()
  await preview.locator('button#primary').click()

  const textField = cssField(page, 'color')
  const textInput = textField.locator('input')

  await expect(textField.getByText('inline style · 1 node')).toBeVisible()
  await expect(textInput).toHaveValue(/#ef4444|rgb\(239, 68, 68\)/)
  await expect(textInput).toBeDisabled()
  await expect.poll(async () =>
    preview.locator('button#primary').evaluate((button) =>
      getComputedStyle(button).color),
  ).toBe('rgb(239, 68, 68)')
  await expect.poll(async () =>
    preview.evaluate((host) =>
      host.shadowRoot?.querySelector('style')?.textContent ?? ''),
  ).toContain('color: #334155 !important;')
})

test('excludes inline style overrides from affected counts', async ({
  page,
}) => {
  await page.goto('/')

  const pasteWasHandled = await page.evaluate((text) => {
    const data = new DataTransfer()

    data.setData('text/plain', text)

    const event = new ClipboardEvent('paste', {
      bubbles: true,
      cancelable: true,
      clipboardData: data,
    })

    window.dispatchEvent(event)

    return event.defaultPrevented
  }, JSON.stringify({
    css: [
      '.button {',
      '  color: #334155;',
      '}',
    ].join('\n'),
    html: [
      '<main>',
      '<button id="primary" class="button">Save</button>',
      '<button id="inline" class="button" style="color: #ef4444;">Inline</button>',
      '</main>',
    ].join(''),
  }))

  expect(pasteWasHandled).toBe(true)

  const preview = page.locator('.demo-html-specimen-preview').last()

  await expect(preview).toBeVisible()
  await preview.locator('button#primary').click()

  const textField = cssField(page, 'color')
  const textInput = textField.locator('input')

  await expect(textField.getByText('.button · 1 node')).toBeVisible()
  await expect(textInput).toHaveValue('#334155')
  await textInput.fill('#111827')
  await textInput.blur()
  await expect.poll(async () =>
    preview.locator('button#primary').evaluate((button) =>
      getComputedStyle(button).color),
  ).toBe('rgb(17, 24, 39)')
  await expect.poll(async () =>
    preview.locator('button#inline').evaluate((button) =>
      getComputedStyle(button).color),
  ).toBe('rgb(239, 68, 68)')
})

test('edits CSS after strings and functions with separator characters', async ({
  page,
}) => {
  await page.goto('/')

  const pasteWasHandled = await page.evaluate((text) => {
    const data = new DataTransfer()

    data.setData('text/plain', text)

    const event = new ClipboardEvent('paste', {
      bubbles: true,
      cancelable: true,
      clipboardData: data,
    })

    window.dispatchEvent(event)

    return event.defaultPrevented
  }, JSON.stringify({
    css: [
      '.primary {',
      '  /* note: ignore ; and } inside comments */',
      '  --asset: url("data:image/svg+xml;utf8,<svg>{}</svg>");',
      '  content: "semi; brace }";',
      '  color: #ffffff;',
      '}',
    ].join('\n'),
    html: [
      '<main>',
      '<button id="primary" class="primary">Save</button>',
      '</main>',
    ].join(''),
  }))

  expect(pasteWasHandled).toBe(true)

  const preview = page.locator('.demo-html-specimen-preview').last()

  await expect(preview).toBeVisible()
  await preview.locator('button#primary').click()

  const textField = cssField(page, 'color')
  const textInput = textField.locator('input')

  await expect(textField.getByText('.primary · 1 node')).toBeVisible()
  await expect(textInput).toHaveValue('#ffffff')
  await textInput.fill('#111827')
  await textInput.blur()
  await expect.poll(async () =>
    preview.evaluate((host) =>
      host.shadowRoot?.querySelector('style')?.textContent ?? ''),
  ).toContain('color: #111827;')
  await expect.poll(async () =>
    preview.locator('button#primary').evaluate((button) =>
      getComputedStyle(button).color),
  ).toBe('rgb(17, 24, 39)')
})

async function readDesignSystemDemoMetrics(page: Page) {
  return page.evaluate(() => {
    const rect = (element: Element | null) => {
      if (!element) {
        return null
      }

      const box = element.getBoundingClientRect()

      return {
        bottom: Math.round(box.bottom),
        height: Math.round(box.height),
        right: Math.round(box.right),
        width: Math.round(box.width),
        x: Math.round(box.x),
        y: Math.round(box.y),
      }
    }
    const specimen = rect(document.querySelector('.demo-html-specimen-shell'))
    const dock = rect(document.querySelector('.devtools-selection-dock'))
    const host = document.querySelector('.demo-html-specimen-preview')
    const root = host instanceof HTMLElement ? host.shadowRoot : null
    const targetElement = root?.querySelector(
      '[data-html-specimen-preview-target]',
    ) ?? null
    const overlayElement = root?.querySelector(
      '[data-html-specimen-preview-overlay="target"]',
    ) ?? null
    const targetRect = rect(targetElement)
    const overlayRect = rect(overlayElement)

    return {
      canvasOrigin: document.querySelector('.devtools-canvas-origin') !== null,
      canvasScale: document.querySelector('.devtools-viewport-controls') !== null,
      dock,
      floatingBar: document.querySelectorAll('.selection-floating-bar').length,
      itemOutline: document.querySelectorAll('.item-outline').length,
      overlayAligned: targetRect && overlayRect
        ? Math.abs(targetRect.x - overlayRect.x) <= 2 &&
          Math.abs(targetRect.y - overlayRect.y) <= 2 &&
          Math.abs(targetRect.width - overlayRect.width) <= 2 &&
          Math.abs(targetRect.height - overlayRect.height) <= 2
        : false,
      resizeHandles: document.querySelectorAll('.resize-handle').length,
      sideInspector: document.querySelectorAll('.object-inspector-devtools')
        .length,
      specimen,
      target: document
        .querySelector('.html-specimen-css-target code')
        ?.textContent
        ?.trim() ?? '',
      toolbar: document.querySelectorAll('[role="toolbar"][aria-label="Tools"]')
        .length,
      zoomControls: document.querySelectorAll('.devtools-viewport-controls')
        .length,
    }
  })
}

function cssField(page: Page, property: string) {
  return page.locator('.html-specimen-css-field').filter({
    has: page.locator('.html-specimen-css-field-label', {
      hasText: new RegExp(`^${escapeRegExp(property)}$`),
    }),
  })
}

async function readCanvasScalePercent(page: Page) {
  const value = await page
    .locator('.devtools-viewport-controls [aria-label="Canvas scale"]')
    .textContent()

  return Number.parseFloat(value ?? '0')
}

function escapeRegExp(value: string) {
  return value.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')
}

async function clickPreviewElement(page: Page, selector: string) {
  const point = await page.evaluate((targetSelector) => {
    const host = document.querySelector('.demo-html-specimen-preview')
    const root = host instanceof HTMLElement ? host.shadowRoot : null
    const element = root?.querySelector(targetSelector)

    if (!element) {
      return null
    }

    const box = element.getBoundingClientRect()

    return {
      x: box.x + box.width / 2,
      y: box.y + box.height / 2,
    }
  }, selector)

  if (!point) {
    throw new Error(`Preview element not found: ${selector}`)
  }

  await page.mouse.click(point.x, point.y)
}

async function dispatchPreviewPointerDown(page: Page, selector: string) {
  const found = await page.evaluate((targetSelector) => {
    const host = document.querySelector('.demo-html-specimen-preview')
    const root = host instanceof HTMLElement ? host.shadowRoot : null
    const element = root?.querySelector(targetSelector)

    if (!element) {
      return false
    }

    element.dispatchEvent(new PointerEvent('pointerdown', {
      bubbles: true,
      composed: true,
    }))

    return true
  }, selector)

  if (!found) {
    throw new Error(`Preview element not found: ${selector}`)
  }
}

async function clickInspectorExport(page: Page, artifact: 'CSS' | 'HTML') {
  await page
    .locator('.html-specimen-actions-strip')
    .getByRole('button', { name: `Export ${artifact}` })
    .click()
}
