...
> = {
  appliedTargets:
    readonly SlideEditObjectTransformPasteAppliedTarget<TObjectId, TTransform>[]
  effects:
    readonly SlideEditObjectTransformHostCommandEffect<
      TSlideId,
      TObjectId,
      TTransform
    >[]
  pasteValue: SlideEditObjectTransformJSONPasteValue
  skippedTargets:
    readonly SlideEditObjectTransformPasteSkippedTarget<TObjectId>[]
}
...
const SLIDE_EDIT_OBJECT_TRANSFORM_FIELDS = Object.freeze([
  ...
  'h',
  'rotation',
] as const satisfies readonly SlideEditObjectTransformFieldId[])
...
>):
  ...
  const appliedTargets:
    SlideEditObjectTransformPasteAppliedTarget<TObjectId, TTransform>[] = []
  const effects:
  ...
  const skippedTargets:
    SlideEditObjectTransformPasteSkippedTarget<TObjectId>[] = []
...
function getSlideEditObjectTransformDirectJSONPasteValue({
  ...
  const fields = SLIDE_EDIT_OBJECT_TRANSFORM_FIELDS.filter(
    (field) => transform[field] !== undefined,
  )
...
function normalizeSlideEditObjectTransformBounds(bounds: Bounds): Bounds {
  return {
...
function roundSlideEditObjectTransformNumber(value: number): number {
  return Math.round(value * 100) / 100
}

function toSlideEditObjectTransformFiniteNumber(value: unknown): number | null {
...
function parseSlideEditObjectTransformJSON(value: string): unknown {
  try {
