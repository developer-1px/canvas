...
> = {
  ...
  kind: 'decrease-list-level' | 'increase-list-level'
  objectId: TObjectId
  slideId: TSlideId
}
...
export function getSlideEditTextParagraphListLevelJSONPasteValue({
  ...
  if (jsonMimeType) {
    const customLevel = getSlideEditTextParagraphListLevelJSONPasteValueFromText(
...
export function normalizeSlideEditTextParagraphListLevel(value: unknown) {
...
export function getSlideEditTextParagraphListLevelModelValue(value: unknown) {
...
function getSlideEditTextParagraphListLevelWrappedJSONValue(value: unknown) {
  ...
  for (const key of SLIDE_EDIT_TEXT_PARAGRAPH_LIST_LEVEL_JSON_WRAPPER_KEYS) {
    ...
    const level = getSlideEditTextParagraphListLevelDirectJSONValue(record[key])
...
function getSlideEditTextParagraphListLevelDirectJSONValue(value: unknown) {
  const numericValue = getSlideEditTextParagraphListLevelNumber(value)
